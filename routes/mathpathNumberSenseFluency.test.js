...
describe('NumberSense fluency routes', () => {
  beforeAll(async () => { const mod = await import('./mathpathNumberSense.js'); router = mod.default; });
  afterEach(() => vi.clearAllMocks());

  it('POST /fluency/start builds a drill and strips answers', async () => {
    MathPathPracticeSession.create.mockResolvedValueOnce({});
    const res = await request('/fluency/start', { method: 'POST', body: { skillId: 'NS001', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('number_sense');
    expect(res.data.skillId).toBe('NS001');
  ...
  it('POST /fluency/:id/submit scores and returns fluencyLevel', async () => {
    const { buildNumberSenseFluencyDrill } = await import('../services/mathpath/numberSenseFluencyService.js');
    const drill = buildNumberSenseFluencyDrill({ skillId: 'NS001', count: 4 });
    const session = { ...drill, targetSkillId: 'NS001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
    ...
    const res = await request('/fluency/x/submit', { method: 'POST', body: { answers, timingsSeconds: drill.questions.map(() => 6) } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('fluency');
