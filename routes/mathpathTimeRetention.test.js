...
describe('Time retention routes', () => {
  beforeAll(async () => { const mod = await import('./mathpathTime.js'); router = mod.default; });
  afterEach(() => vi.clearAllMocks());

  it('GET /retention returns domainId and buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [{ skillId: 'TM001', domainId: 'time', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' }]),
    });
    const res = await request('/retention', { method: 'GET' });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('time');
  ...
  it('POST /retention/start builds a review and strips answers', async () => {
    ...
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'TM001' } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('time');
  ...
  it('POST /retention/:id/submit scores and persists', async () => {
    const { buildTimeRetentionReview } = await import('../services/mathpath/timeRetentionService.js');
    const built = buildTimeRetentionReview({ skillId: 'TM001' });
    const session = { ...built, targetSkillId: 'TM001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
