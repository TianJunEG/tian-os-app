...
const CODE_TO_SLUG = Object.fromEntries(
  (statisticsSkillGraph.skills || []).map((s) => [s.id, s.slug])
);
...
router.post('/practice/start', protect, async (req, res) => {
  try {
    ...
    const built = buildStatisticsPracticeSession({ targetSkillId, masteredSkillIds, weakSkillIds, questionCount });
    ...
    await MathPathPracticeSession.create({
      ...
      sessionGoal: 'Statistics practice', estimatedQuestionCount: built.questions.length,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start statistics practice.' });
  }
});
...
router.post('/practice/:practiceSessionId/submit', protect, async (req, res) => {
  try {
    ...
    if (!existing) return res.status(404).json({ error: 'Statistics practice session not found.' });
    ...
    const scored = scoreStatisticsSubmission({ questions: existing.questions || [], responses });
    ...
    for (const mistake of scored.mistakes) {
      const tag = mistake.misconceptionTag || 'statistics_error';
      await MathPathMistakeRecord.findOneAndUpdate(
        { studentId, domainId: DOMAIN_ID, mistakeCode: tag, skillId: mistake.skillId || '', questionFamilyId: mistake.questionFamilyId || '' },
        {
          ...
          $push: { evidence: {
            source: 'statistics-practice-incorrect', questionId: mistake.questionId,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit statistics practice.' });
  }
});
