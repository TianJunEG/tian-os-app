...
describe('Circles retention routes', () => {
  ...
  it('GET /retention returns domainId and buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [{ skillId: 'CI001', domainId: 'circles', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' }]),
    });
  ...
  it('POST /retention/start builds a review and strips answers', async () => {
    ...
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'CI001' } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('circles');
  ...
  it('POST /retention/:id/submit scores and persists', async () => {
    ...
    const built = buildCirclesRetentionReview({ skillId: 'CI001' });
    const session = { ...built, targetSkillId: 'CI001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { answers, timingsSeconds: built.questions.map(() => 10) } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('retention');
