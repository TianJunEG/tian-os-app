...
const question = { findById: vi.fn(), find: vi.fn() };
...
vi.mock('../services/diagnostics/diagnosticDomainRegistry.js', () => ({
  getDiagnosticDomain: (...a) => getDiagnosticDomain(...a),
}));
...
describe('kiosk resume (mid-attempt refresh)', () => {
  beforeAll(async () => {
    process.env.JWT_SECRET = 'test-secret-kiosk-routes';
    router = (await import('./kioskDiagnostics.js')).default;
  });
  ...
  it('diagnostic: rehydrates the current question, scrubbed of its answer', async () => {
    // Mint a real diagnostic token.
    cds.findOne.mockResolvedValueOnce(makeSession());
    student.findById.mockResolvedValueOnce({ _id: AISHA, workspaceId: 'ws_1', level: 'P4' });
    cds.findOneAndUpdate.mockResolvedValueOnce(makeSession());
    cds.updateOne.mockResolvedValue({});
    mpds.updateOne.mockResolvedValue({});
    startAdaptiveDiagnostic.mockResolvedValueOnce({ sessionId: 'fractionsdiag_r', currentQuestion: {}, questions: [], progress: {} });
    const token = (await request('/sessions/K7Q4MN/begin', { method: 'POST', body: { ref: '0' } })).data.attemptToken;

    // loadAttempt awaits findById; resume then calls findById().lean().
    cds.findById.mockResolvedValueOnce(makeSession());
    cds.findById.mockReturnValueOnce({ lean: () => Promise.resolve(makeSession()) });
    student.findById.mockResolvedValueOnce({ _id: AISHA, workspaceId: 'ws_1' });
    mpds.findOne.mockReturnValueOnce({ lean: () => Promise.resolve({
      diagnosticSessionId: 'fractionsdiag_r', status: 'active', currentQuestionId: 'q1',
      subjectId: 'math', domainId: 'fractions', adaptiveState: { responses: [{}, {}], maxQuestions: 10 },
    }) });
    getDiagnosticDomain.mockReturnValueOnce({
      getQuestionById: async () => ({ skillId: 'F001' }),
      normaliseQuestion: () => ({ questionId: 'q1', prompt: '2 + 2 = ?', type: 'short_answer', choices: [], answer: '4' }),
    });

    const res = await request('/attempt/fractionsdiag_r/resume', { method: 'GET', headers: { 'x-attempt-token': token } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('diagnostic');
    expect(res.data.currentQuestion).toMatchObject({ questionId: 'q1', prompt: '2 + 2 = ?' });
    expect(res.data.currentQuestion.answer).toBeUndefined(); // no answer leaked on resume
    expect(res.data.progress.answeredCount).toBe(2);
  });

  it('practice: rehydrates the fixed set + how many are already answered', async () => {
    // Mint a real practice token.
    cds.findOne.mockResolvedValueOnce(makePracticeSession());
    student.findById.mockResolvedValueOnce({ _id: AISHA, workspaceId: 'ws_1' });
    selectSimilarQuestions.mockResolvedValueOnce([{ _id: 'q1', stem: '10 + 5', type: 'short_answer', choices: [], answer: '15', skillId: 'sk_add2' }]);
    practiceSession.create.mockResolvedValueOnce({ _id: 'ps_r' });
    cds.updateOne.mockResolvedValue({});
    const token = (await request('/sessions/K7Q4MN/practice-begin', { method: 'POST', body: { ref: '0' } })).data.attemptToken;

    cds.findById.mockResolvedValueOnce(makePracticeSession()); // loadAttempt
    cds.findById.mockReturnValueOnce({ lean: () => Promise.resolve(makePracticeSession()) }); // resume classSession
    student.findById.mockResolvedValueOnce({ _id: AISHA, workspaceId: 'ws_1' });
    practiceSession.findById.mockReturnValueOnce({ lean: () => Promise.resolve({ status: 'active', questionIds: ['q1', 'q2'] }) });
    question.find.mockReturnValueOnce({ populate: () => Promise.resolve([
      { _id: 'q1', stem: '10 + 5', type: 'short_answer', choices: [] },
      { _id: 'q2', stem: '20 + 5', type: 'short_answer', choices: [] },
    ]) });
    practiceAttempt.find.mockReturnValueOnce({ select: () => ({ lean: () => Promise.resolve([{ questionId: 'q1' }]) }) });

    const res = await request('/attempt/ps_r/resume', { method: 'GET', headers: { 'x-attempt-token': token } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('practice');
    expect(res.data.items).toHaveLength(2);
    expect(res.data.answeredCount).toBe(1);
    expect(res.data.skillName).toBe('2-digit addition');
  });

  it('rejects resume with a mismatched token / closed parent session', async () => {
    // Mint a token bound to fractionsdiag_r, then hit a different session id → 403.
    cds.findOne.mockResolvedValueOnce(makeSession());
    student.findById.mockResolvedValueOnce({ _id: AISHA, workspaceId: 'ws_1', level: 'P4' });
    cds.findOneAndUpdate.mockResolvedValueOnce(makeSession());
    cds.updateOne.mockResolvedValue({});
    ...
    const mismatch = await request('/attempt/OTHER/resume', { method: 'GET', headers: { 'x-attempt-token': token } });
    expect(mismatch.status).toBe(403);

    // valid token but the teacher has since closed the check-in → 410
    cds.findById.mockResolvedValueOnce(makeSession({ status: 'closed' }));
    const closed = await request('/attempt/fractionsdiag_r/resume', { method: 'GET', headers: { 'x-attempt-token': token } });
