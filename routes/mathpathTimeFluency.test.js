...
describe('Time fluency routes', () => {
  ...
  it('POST /fluency/start builds a drill and strips answers', async () => {
    ...
    const res = await request('/fluency/start', { method: 'POST', body: { skillId: 'TM001', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('time');
    expect(res.data.skillId).toBe('TM001');
  ...
  it('POST /fluency/:id/submit scores and returns fluencyLevel', async () => {
    const { buildTimeFluencyDrill } = await import('../services/mathpath/timeFluencyService.js');
    const drill = buildTimeFluencyDrill({ skillId: 'TM001', count: 4 });
    const session = { ...drill, targetSkillId: 'TM001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
