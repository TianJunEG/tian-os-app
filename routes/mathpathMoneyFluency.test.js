...
describe('Money fluency routes', () => {
  ...
  it('POST /fluency/start builds a drill and strips answers', async () => {
    ...
    const res = await request('/fluency/start', { method: 'POST', body: { skillId: 'MN001', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('money');
    expect(res.data.skillId).toBe('MN001');
  ...
  it('POST /fluency/:id/submit scores and returns fluencyLevel', async () => {
    const { buildMoneyFluencyDrill } = await import('../services/mathpath/moneyFluencyService.js');
    const drill = buildMoneyFluencyDrill({ skillId: 'MN001', count: 4 });
    const session = { ...drill, targetSkillId: 'MN001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
