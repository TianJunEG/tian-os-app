...
describe('Algebra retention routes', () => {
  beforeAll(async () => {
    const mod = await import('./mathpathAlgebra.js');
    router = mod.default;
  });
  ...
  it('GET /retention returns the upcoming/overdue/history buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [
        { skillId: 'AL003', domainId: 'algebra', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
      ]),
    });
    const res = await request('/retention', { method: 'GET' });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('algebra');
  ...
  it('POST /retention/start builds + persists a review and strips answers', async () => {
    MathPathPracticeSession.create.mockResolvedValueOnce({ practiceSessionId: 'algebraretention_1' });
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'AL003', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('algebra');
    expect(res.data.skillId).toBe('AL003');
  ...
  it('POST /retention/:id/submit scores the review and persists the schedule patch', async () => {
    // Build a real review first so we have valid question ids + answers.
    const { buildAlgebraRetentionReview } = await import('../services/mathpath/algebraRetentionService.js');
    const built = buildAlgebraRetentionReview({ skillId: 'AL003', count: 4 });
    const session = {
      practiceSessionId: 'algebraretention_1',
      domainId: 'algebra',
      status: 'inProgress',
      targetSkillId: 'AL003',
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { responses, intervalDays: 3 }, params: { practiceSessionId: 'algebraretention_1' } });
