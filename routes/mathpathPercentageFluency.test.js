...
const MOCK_STUDENT = { _id: 'student_pct_1' };
...
describe('Percentage fluency endpoints', () => {
  beforeAll(async () => {
  ...
  it('starts a fluency drill and returns answer-stripped questions', async () => {
    const res = await request('/fluency/start', { method: 'POST', body: { skillId: 'P001', count: 6 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('percentage');
    expect(res.data.skillId).toBe('P001');
    ...
    expect(storedSession.domainId).toBe('percentage');
  });
  ...
  it('scores a submitted drill into a fluency band and persists it', async () => {
    await request('/fluency/start', { method: 'POST', body: { skillId: 'P001', count: 6 } });
    ...
    expect(filter.domainId).toBe('percentage');
    expect(filter.skillId).toBe('P001');
  });
});
