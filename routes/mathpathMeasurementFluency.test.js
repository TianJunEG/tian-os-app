...
describe('Measurement fluency routes', () => {
  ...
  it('POST /fluency/start builds a drill and strips answers', async () => {
    ...
    const res = await request('/fluency/start', { method: 'POST', body: { skillId: 'ME001', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('measurement');
    expect(res.data.skillId).toBe('ME001');
  ...
  it('POST /fluency/:id/submit scores and returns fluencyLevel', async () => {
    const { buildMeasurementFluencyDrill } = await import('../services/mathpath/measurementFluencyService.js');
    const drill = buildMeasurementFluencyDrill({ skillId: 'ME001', count: 4 });
    const session = { ...drill, targetSkillId: 'ME001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
