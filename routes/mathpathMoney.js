...
import { buildMoneyFluencyDrill, toClientFluencyQuestions, scoreMoneyFluencyDrill } from '../services/mathpath/moneyFluencyService.js';
import { buildRetentionScheduleFromFluency, summariseRetention } from '../shared/mathpath/money/moneyRetentionEngine.js';
import { buildMoneyRetentionReview, toClientRetentionQuestions, scoreMoneyRetentionReview } from '../services/mathpath/moneyRetentionService.js';
...
router.post('/fluency/start', protect, async (req, res) => {
  try {
    ...
    const drill = buildMoneyFluencyDrill({ skillId, studentId, count });
    const sessionId = `moneyfluency_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await MathPathPracticeSession.create({
      practiceSessionId: sessionId, studentId, domainId: DOMAIN_ID,
      targetSkillId: skillId, mode: 'fluency',
      sessionGoal: 'Money fluency drill', estimatedQuestionCount: drill.totalQuestions,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start money fluency drill.' });
  }
});
...
router.post('/fluency/:id/submit', protect, async (req, res) => {
  try {
    ...
    const scored = scoreMoneyFluencyDrill({ drill: existing.toObject(), answers, timingsSeconds });
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit money fluency drill.' });
  }
});
...
router.get('/retention', protect, async (req, res) => {
  try {
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to load money retention summary.' });
  }
});
...
router.post('/retention/start', protect, async (req, res) => {
  try {
    ...
    if (!skillId) return res.status(400).json({ error: 'skillId required.' });
    const review = buildMoneyRetentionReview({ skillId, studentId, previousQuestionFamilyIds, difficulty });
    const sessionId = `moneyretention_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await MathPathPracticeSession.create({
      practiceSessionId: sessionId, studentId, domainId: DOMAIN_ID,
      targetSkillId: skillId, mode: 'retention',
      sessionGoal: 'Money retention review', estimatedQuestionCount: review.totalQuestions,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start money retention review.' });
  }
});
...
router.post('/retention/:id/submit', protect, async (req, res) => {
  try {
    ...
    const { answers = [], timingsSeconds = [] } = req.body || {};
    const scored = scoreMoneyRetentionReview({ review: { ...existing.toObject(), mode: 'retention' }, answers, timingsSeconds });
    if (scored.set) {
      await MathPathStudentSkillState.findOneAndUpdate(
        { studentId, domainId: DOMAIN_ID, skillId: existing.targetSkillId },
        { $set: scored.set },
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit money retention review.' });
  }
});
