...
describe('Percentage retention routes', () => {
  ...
  it('GET /retention returns the upcoming/overdue/history buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [
        { skillId: 'P003', domainId: 'percentage', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
      ]),
    });
  ...
  it('POST /retention/start builds + persists a review and strips answers', async () => {
    MathPathPracticeSession.create.mockResolvedValueOnce({ practiceSessionId: 'percentageretention_1' });
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'P003', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('percentage');
    expect(res.data.skillId).toBe('P003');
  ...
  it('POST /retention/:id/submit scores the review and persists the schedule patch', async () => {
    ...
    const { buildPercentageRetentionReview } = await import('../services/mathpath/percentageRetentionService.js');
    const built = buildPercentageRetentionReview({ skillId: 'P003', count: 4 });
    const session = {
      practiceSessionId: 'percentageretention_1',
      domainId: 'percentage',
      status: 'inProgress',
      targetSkillId: 'P003',
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { responses, intervalDays: 3 }, params: { practiceSessionId: 'percentageretention_1' } });
