...
import {
  buildPercentageFluencyDrill,
  toClientFluencyQuestions,
  scorePercentageFluencyDrill,
} from '../services/mathpath/percentageFluencyService.js';
import {
  buildPercentageRetentionReview,
  toClientRetentionQuestions,
  scorePercentageRetentionReview,
} from '../services/mathpath/percentageRetentionService.js';
import {
...
// @route POST /api/mathpath/percentages/fluency/start
...
router.post('/fluency/start', protect, async (req, res) => {
  try {
    ...
    const drill = buildPercentageFluencyDrill({ skillId, count });
    const practiceSessionId = `percentagefluency_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ...
    await MathPathPracticeSession.create({
      ...
      sessionGoal: 'Percentage fluency', estimatedQuestionCount: drill.questions.length,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start percentage fluency drill.' });
  }
});

// @route POST /api/mathpath/percentages/fluency/:practiceSessionId/submit
...
router.post('/fluency/:practiceSessionId/submit', protect, async (req, res) => {
  try {
    ...
    const studentId = String(student._id);
    const existing = await MathPathPracticeSession.findOne({ practiceSessionId: req.params.practiceSessionId, studentId });
    if (!existing) return res.status(404).json({ error: 'Percentage fluency drill not found.' });
    if (existing.domainId !== DOMAIN_ID) return res.status(400).json({ error: 'Session is not a percentage session.' });
    if (existing.status === 'completed') return res.json({ ...(existing.summary || {}), alreadyCompleted: true });

    const responses = Array.isArray(req.body?.responses) ? req.body.responses : [];
    const scored = scorePercentageFluencyDrill({ skillId: existing.targetSkillId, questions: existing.questions || [], responses });
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit percentage fluency drill.' });
  }
});

// @route GET /api/mathpath/percentages/retention
...
router.get('/retention', protect, async (req, res) => {
  try {
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to load percentage retention reviews.' });
  }
});

// @route POST /api/mathpath/percentages/retention/start
// @desc  Build + persist a spaced retention review; returns answer-stripped questions.
router.post('/retention/start', protect, async (req, res) => {
  try {
    ...
    const review = buildPercentageRetentionReview({ skillId, previousQuestionFamilyIds, count });
    const practiceSessionId = `percentageretention_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

    await MathPathPracticeSession.create({
      practiceSessionId, studentId, domainId: DOMAIN_ID,
      targetSkillId: skillId,
      targetQuestionFamilyIds: [...new Set(review.questions.map((q) => q.questionFamilyId))],
      sessionGoal: 'Percentage retention review', estimatedQuestionCount: review.questions.length,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start percentage retention review.' });
  }
});

// @route POST /api/mathpath/percentages/retention/:practiceSessionId/submit
...
router.post('/retention/:practiceSessionId/submit', protect, async (req, res) => {
  try {
    ...
    const studentId = String(student._id);
    const existing = await MathPathPracticeSession.findOne({ practiceSessionId: req.params.practiceSessionId, studentId });
    if (!existing) return res.status(404).json({ error: 'Percentage retention review not found.' });
    if (existing.domainId !== DOMAIN_ID) return res.status(400).json({ error: 'Session is not a percentage session.' });
    if (existing.status === 'completed') return res.json({ ...(existing.summary || {}), alreadyCompleted: true });
    ...
    const scored = scorePercentageRetentionReview({
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit percentage retention review.' });
  }
});
