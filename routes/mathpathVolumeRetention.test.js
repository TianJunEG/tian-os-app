...
describe('Volume retention routes', () => {
  beforeAll(async () => {
    const mod = await import('./mathpathVolume.js');
    router = mod.default;
  });
  ...
  it('GET /retention returns the upcoming/overdue/history buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [
        { skillId: 'VL002', domainId: 'volume', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
      ]),
    });
  ...
  it('POST /retention/start builds + persists a review and strips answers', async () => {
    MathPathPracticeSession.create.mockResolvedValueOnce({ practiceSessionId: 'volumeretention_1' });
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'VL002', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('volume');
  ...
  it('POST /retention/:id/submit scores the review and persists the schedule patch', async () => {
    ...
    const { buildVolumeRetentionReview } = await import('../services/mathpath/volumeRetentionService.js');
    const built = buildVolumeRetentionReview({ skillId: 'VL002', count: 4 });
    const session = {
      practiceSessionId: 'volumeretention_1',
      domainId: 'volume',
      status: 'inProgress',
      targetSkillId: 'VL002',
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { responses, intervalDays: 3 }, params: { practiceSessionId: 'volumeretention_1' } });
