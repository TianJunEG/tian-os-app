...
import { sendPasswordResetEmail, sendVerificationEmail, sendWelcomeEmail, appBaseUrl } from '../utils/emailService.js';
...
// @desc    A student creates their own login (own email + password). Distinct
//          from /register, which only ever covers parent/tutor and is gated
//          by openRegistration — this is its own rollout flag since letting a
//          minor sign themselves up is a separate decision from that one.
// @access  Public (behind FEAT_STUDENT_SIGNUP)
router.post(
  '/student-signup',
  authRateLimit,
  [
  ...
  async (req, res) => {
    if (process.env.FEAT_STUDENT_SIGNUP !== '1') {
      return res.status(403).json({
        error: 'student_signup_closed',
        message: 'Student sign-up is not open yet. Ask your parent, tutor, or teacher for a login.',
      });
    }
    ...
    try {
      ...
      // Account is usable right away (auto-login below) — verification just
      // confirms the email is real so a later "forgot password" actually
      // reaches them. It doesn't gate access, so a slow/missed email can't
      // lock a student out.
      const verifyUrl = `${appBaseUrl()}/verify-email/${verifyToken}`;
...
router.post('/verify-email/:token', authRateLimit, asyncHandler(async (req, res) => {
  const hashed = crypto.createHash('sha256').update(req.params.token).digest('hex');
  const user = await User.findOne({
...
router.post('/resend-verification', protect, authRateLimit, asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id);
