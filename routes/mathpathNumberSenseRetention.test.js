...
describe('NumberSense retention routes', () => {
  ...
  it('GET /retention returns domainId and buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [{ skillId: 'NS001', domainId: 'number_sense', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' }]),
    });
  ...
  it('POST /retention/start builds a review and strips answers', async () => {
    ...
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'NS001' } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('number_sense');
  ...
  it('POST /retention/:id/submit scores and persists', async () => {
    const { buildNumberSenseRetentionReview } = await import('../services/mathpath/numberSenseRetentionService.js');
    const built = buildNumberSenseRetentionReview({ skillId: 'NS001' });
