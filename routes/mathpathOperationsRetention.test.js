...
describe('Operations retention routes', () => {
  beforeAll(async () => { const mod = await import('./mathpathOperations.js'); router = mod.default; });
  afterEach(() => vi.clearAllMocks());

  it('GET /retention returns domainId and buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [{ skillId: 'OP001', domainId: 'four_operations', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' }]),
    });
    const res = await request('/retention', { method: 'GET' });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('four_operations');
  ...
  it('POST /retention/start builds a review and strips answers', async () => {
    ...
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'OP001' } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('four_operations');
  ...
  it('POST /retention/:id/submit scores and persists', async () => {
    const { buildOperationsRetentionReview } = await import('../services/mathpath/operationsRetentionService.js');
    const built = buildOperationsRetentionReview({ skillId: 'OP001' });
    const session = { ...built, targetSkillId: 'OP001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { answers, timingsSeconds: built.questions.map(() => 5) } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('retention');
