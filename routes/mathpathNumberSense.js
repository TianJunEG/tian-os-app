...
import {
  DOMAIN_ID, buildNumberSensePracticeSession, toClientQuestions, scoreNumberSenseSubmission,
} from '../services/mathpath/numberSensePracticeService.js';
...
import { buildNumberSenseFluencyDrill, toClientFluencyQuestions, scoreNumberSenseFluencyDrill } from '../services/mathpath/numberSenseFluencyService.js';
import { buildRetentionScheduleFromFluency, summariseRetention } from '../shared/mathpath/numberSense/numberSenseRetentionEngine.js';
import { buildNumberSenseRetentionReview, toClientRetentionQuestions, scoreNumberSenseRetentionReview } from '../services/mathpath/numberSenseRetentionService.js';

const CODE_TO_SLUG = Object.fromEntries(
  (numberSenseSkillGraph.skills || []).map((s) => [s.id, s.slug])
);
...
function newSessionId() {
  return `numberSensepractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}
...
router.post('/practice/start', protect, async (req, res) => {
  try {
    ...
    const built = buildNumberSensePracticeSession({ targetSkillId, masteredSkillIds, weakSkillIds, questionCount });
    ...
    await MathPathPracticeSession.create({
      ...
      sessionGoal: 'NumberSense practice', estimatedQuestionCount: built.questions.length,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start number-sense practice.' });
  }
});
...
router.post('/practice/:practiceSessionId/submit', protect, async (req, res) => {
  try {
    ...
    if (!existing) return res.status(404).json({ error: 'NumberSense practice session not found.' });
    if (existing.domainId !== DOMAIN_ID) return res.status(400).json({ error: 'Session is not a number-sense session.' });
    if (existing.status === 'completed') return res.json({ ...(existing.summary || {}), alreadyCompleted: true });
    const responses = Array.isArray(req.body?.responses) ? req.body.responses : [];
    const scored = scoreNumberSenseSubmission({ questions: existing.questions || [], responses });
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit number-sense practice.' });
  }
});
...
router.get('/skill-states', protect, async (req, res) => {
  try {
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to load number-sense skill states.' });
  }
});
...
router.post('/fluency/start', protect, async (req, res) => {
  try {
    ...
    const drill = buildNumberSenseFluencyDrill({ skillId, studentId, count });
    const sessionId = `numberSensefluency_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await MathPathPracticeSession.create({
      practiceSessionId: sessionId, studentId, domainId: DOMAIN_ID,
      targetSkillId: skillId, mode: 'fluency',
      sessionGoal: 'Number Sense fluency drill', estimatedQuestionCount: drill.totalQuestions,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start number sense fluency drill.' });
  }
});
...
router.post('/fluency/:id/submit', protect, async (req, res) => {
  try {
    ...
    const scored = scoreNumberSenseFluencyDrill({ drill: existing.toObject(), answers, timingsSeconds });
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit number sense fluency drill.' });
  }
});
...
router.get('/retention', protect, async (req, res) => {
  try {
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to load number sense retention summary.' });
  }
});
...
router.post('/retention/start', protect, async (req, res) => {
  try {
    ...
    if (!skillId) return res.status(400).json({ error: 'skillId required.' });
    const review = buildNumberSenseRetentionReview({ skillId, studentId, previousQuestionFamilyIds, difficulty });
    const sessionId = `numberSenseretention_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await MathPathPracticeSession.create({
      practiceSessionId: sessionId, studentId, domainId: DOMAIN_ID,
      targetSkillId: skillId, mode: 'retention',
      sessionGoal: 'Number Sense retention review', estimatedQuestionCount: review.totalQuestions,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start number sense retention review.' });
  }
});
...
router.post('/retention/:id/submit', protect, async (req, res) => {
  try {
    ...
    const { answers = [], timingsSeconds = [] } = req.body || {};
    const scored = scoreNumberSenseRetentionReview({ review: { ...existing.toObject(), mode: 'retention' }, answers, timingsSeconds });
    if (scored.set) {
      await MathPathStudentSkillState.findOneAndUpdate(
        { studentId, domainId: DOMAIN_ID, skillId: existing.targetSkillId },
        { $set: scored.set },
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit number sense retention review.' });
  }
});
