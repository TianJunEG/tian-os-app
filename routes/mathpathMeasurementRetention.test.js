...
describe('Measurement retention routes', () => {
  beforeAll(async () => { const mod = await import('./mathpathMeasurement.js'); router = mod.default; });
  afterEach(() => vi.clearAllMocks());

  it('GET /retention returns domainId and buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [{ skillId: 'ME001', domainId: 'measurement', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' }]),
    });
    const res = await request('/retention', { method: 'GET' });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('measurement');
  ...
  it('POST /retention/start builds a review and strips answers', async () => {
    ...
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'ME001' } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('measurement');
  ...
  it('POST /retention/:id/submit scores and persists', async () => {
    const { buildMeasurementRetentionReview } = await import('../services/mathpath/measurementRetentionService.js');
    const built = buildMeasurementRetentionReview({ skillId: 'ME001' });
    const session = { ...built, targetSkillId: 'ME001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
