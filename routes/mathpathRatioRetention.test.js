...
const MathPathStudentSkillState = {
  ...
  findOne: vi.fn(() => ({ lean: vi.fn(async () => null) })),  // REQUIRED
...
describe('Ratio retention routes', () => {
  beforeAll(async () => {
  ...
  it('GET /retention returns the upcoming/overdue/history buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [
        { skillId: 'R003', domainId: 'ratio', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
      ]),
    });
  ...
  it('POST /retention/start builds + persists a review and strips answers', async () => {
    MathPathPracticeSession.create.mockResolvedValueOnce({ practiceSessionId: 'ratioretention_1' });
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'R003', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('ratio');
    expect(res.data.skillId).toBe('R003');
  ...
  it('POST /retention/:id/submit scores the review and persists the schedule patch', async () => {
    ...
    const { buildRatioRetentionReview } = await import('../services/mathpath/ratioRetentionService.js');
    const built = buildRatioRetentionReview({ skillId: 'R003', count: 4 });
    const session = {
      practiceSessionId: 'ratioretention_1',
      domainId: 'ratio',
      status: 'inProgress',
      targetSkillId: 'R003',
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { responses, intervalDays: 3 }, params: { practiceSessionId: 'ratioretention_1' } });
