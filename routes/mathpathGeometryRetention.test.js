...
describe('Geometry retention routes', () => {
  beforeAll(async () => {
    const mod = await import('./mathpathGeometry.js');
    router = mod.default;
  });
  ...
  it('GET /retention returns the upcoming/overdue/history buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [
        { skillId: 'GE003', domainId: 'geometry', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
      ]),
    });
  ...
  it('POST /retention/start builds + persists a review and strips answers', async () => {
    MathPathPracticeSession.create.mockResolvedValueOnce({ practiceSessionId: 'geometryretention_1' });
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'GE003', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('geometry');
    expect(res.data.skillId).toBe('GE003');
  ...
  it('POST /retention/:id/submit scores the review and persists the schedule patch', async () => {
    ...
    const { buildGeometryRetentionReview } = await import('../services/mathpath/geometryRetentionService.js');
    const built = buildGeometryRetentionReview({ skillId: 'GE003', count: 4 });
    const session = {
      practiceSessionId: 'geometryretention_1',
      domainId: 'geometry',
      status: 'inProgress',
      targetSkillId: 'GE003',
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { responses, intervalDays: 3 }, params: { practiceSessionId: 'geometryretention_1' } });
