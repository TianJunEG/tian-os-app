...
describe('Statistics retention routes', () => {
  ...
  it('GET /retention returns domainId and buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [{ skillId: 'ST001', domainId: 'statistics', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' }]),
    });
  ...
  it('POST /retention/start builds a review and strips answers', async () => {
    ...
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'ST001' } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('statistics');
  ...
  it('POST /retention/:id/submit scores and persists', async () => {
    const { buildStatisticsRetentionReview } = await import('../services/mathpath/statisticsRetentionService.js');
    const built = buildStatisticsRetentionReview({ skillId: 'ST001' });
    const session = { domainId: 'statistics', targetSkillId: 'ST001', status: 'inProgress', questions: built.questions, toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
    MathPathPracticeSession.findOne.mockResolvedValueOnce(session);
    const responses = built.questions.map((q) => ({ questionId: q.questionId, studentAnswer: String(q.answer ?? q.acceptedAnswers?.[0] ?? ''), timeTaken: 5 }));
