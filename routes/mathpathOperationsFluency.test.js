...
describe('Operations fluency routes', () => {
  ...
  it('POST /fluency/start builds a drill and strips answers', async () => {
    ...
    const res = await request('/fluency/start', { method: 'POST', body: { skillId: 'OP001', count: 4 } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('four_operations');
    expect(res.data.skillId).toBe('OP001');
  ...
  it('POST /fluency/:id/submit scores and returns fluencyLevel', async () => {
    const { buildOperationsFluencyDrill } = await import('../services/mathpath/operationsFluencyService.js');
    const drill = buildOperationsFluencyDrill({ skillId: 'OP001', count: 4 });
    const session = { ...drill, targetSkillId: 'OP001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
    ...
    const res = await request('/fluency/x/submit', { method: 'POST', body: { answers, timingsSeconds: drill.questions.map(() => 5) } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('fluency');
