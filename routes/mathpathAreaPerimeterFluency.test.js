...
describe('AreaPerimeter fluency routes', () => {
  ...
  it('POST /fluency/start builds a drill and strips answers', async () => {
    ...
    const res = await request('/fluency/start', { method: 'POST', body: { skillId: 'AP001', count: 4 } });
  ...
  it('POST /fluency/:id/submit scores the drill and returns fluencyLevel', async () => {
    const { buildAreaPerimeterFluencyDrill } = await import('../services/mathpath/areaPerimeterFluencyService.js');
    const drill = buildAreaPerimeterFluencyDrill({ skillId: 'AP001', count: 4 });
    const session = { ...drill, targetSkillId: 'AP001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
    ...
    const res = await request('/fluency/x/submit', { method: 'POST', body: { answers, timingsSeconds: drill.questions.map(() => 8) } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('fluency');
  ...
  it('POST /fluency/:id/submit 404s for unknown session', async () => {
    ...
    const res = await request('/fluency/nope/submit', { method: 'POST', body: { answers: [] } });
