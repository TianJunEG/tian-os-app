...
describe('AreaPerimeter retention routes', () => {
  beforeAll(async () => { const mod = await import('./mathpathAreaPerimeter.js'); router = mod.default; });
  afterEach(() => vi.clearAllMocks());

  it('GET /retention returns domainId and buckets', async () => {
    MathPathStudentSkillState.find.mockReturnValueOnce({
      lean: vi.fn(async () => [{ skillId: 'AP001', domainId: 'area_perimeter', fluentAt: '2026-01-01T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' }]),
    });
    const res = await request('/retention', { method: 'GET' });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('area_perimeter');
  ...
  it('POST /retention/start builds a review and strips answers', async () => {
    ...
    const res = await request('/retention/start', { method: 'POST', body: { skillId: 'AP001' } });
    expect(res.status).toBe(200);
    expect(res.data.domainId).toBe('area_perimeter');
    expect(res.data.skillId).toBe('AP001');
  ...
  it('POST /retention/:id/submit scores and persists', async () => {
    const { buildAreaPerimeterRetentionReview } = await import('../services/mathpath/areaPerimeterRetentionService.js');
    const built = buildAreaPerimeterRetentionReview({ skillId: 'AP001' });
    const session = { ...built, targetSkillId: 'AP001', status: 'inProgress', toObject() { return { ...this }; }, save: vi.fn(async function () { return this; }) };
    ...
    const res = await request('/retention/x/submit', { method: 'POST', body: { answers, timingsSeconds: built.questions.map(() => 8) } });
    expect(res.status).toBe(200);
    expect(res.data.mode).toBe('retention');
