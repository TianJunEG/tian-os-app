...
import { buildOperationsFluencyDrill, toClientFluencyQuestions, scoreOperationsFluencyDrill } from '../services/mathpath/operationsFluencyService.js';
import { buildRetentionScheduleFromFluency, summariseRetention } from '../shared/mathpath/operations/operationsRetentionEngine.js';
import { buildOperationsRetentionReview, toClientRetentionQuestions, scoreOperationsRetentionReview } from '../services/mathpath/operationsRetentionService.js';
...
router.post('/fluency/start', protect, async (req, res) => {
  try {
    ...
    const drill = buildOperationsFluencyDrill({ skillId, studentId, count });
    const sessionId = `operationsfluency_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await MathPathPracticeSession.create({
      practiceSessionId: sessionId, studentId, domainId: DOMAIN_ID,
      targetSkillId: skillId, mode: 'fluency',
      sessionGoal: 'Operations fluency drill', estimatedQuestionCount: drill.totalQuestions,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start operations fluency drill.' });
  }
});
...
router.post('/fluency/:id/submit', protect, async (req, res) => {
  try {
    ...
    const scored = scoreOperationsFluencyDrill({ drill: existing.toObject(), answers, timingsSeconds });
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit operations fluency drill.' });
  }
});
...
router.get('/retention', protect, async (req, res) => {
  try {
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to load operations retention summary.' });
  }
});
...
router.post('/retention/start', protect, async (req, res) => {
  try {
    ...
    if (!skillId) return res.status(400).json({ error: 'skillId required.' });
    const review = buildOperationsRetentionReview({ skillId, studentId, previousQuestionFamilyIds, difficulty });
    const sessionId = `operationsretention_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await MathPathPracticeSession.create({
      practiceSessionId: sessionId, studentId, domainId: DOMAIN_ID,
      targetSkillId: skillId, mode: 'retention',
      sessionGoal: 'Operations retention review', estimatedQuestionCount: review.totalQuestions,
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start operations retention review.' });
  }
});
...
router.post('/retention/:id/submit', protect, async (req, res) => {
  try {
    ...
    const { answers = [], timingsSeconds = [] } = req.body || {};
    const scored = scoreOperationsRetentionReview({ review: { ...existing.toObject(), mode: 'retention' }, answers, timingsSeconds });
    if (scored.set) {
      await MathPathStudentSkillState.findOneAndUpdate(
        { studentId, domainId: DOMAIN_ID, skillId: existing.targetSkillId },
        { $set: scored.set },
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to submit operations retention review.' });
  }
});
