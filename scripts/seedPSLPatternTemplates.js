...
const TEMPLATES = [
  ...
  mt('psl-tpl-pat-repeat-01', 'psl-p4-pat-repeated', {
    ...
    constraints: {
      ...
      _compute: {
        position: '((n - 1) % cycleLength) + 1',
        answer: '((n - 1) % cycleLength) + 1',
      },
    },
  ...
  mt('psl-tpl-pat-repeat-02', 'psl-p4-pat-repeated', {
    ...
    constraints: {
      ...
      _compute: {
        fullCycles: 'Math.floor(n / cycleLength)',
        remainder: 'n % cycleLength',
        answer: 'Math.floor(n / cycleLength) + (n % cycleLength > 0 ? 1 : 0)',
      },
    },
  ...
  mt('psl-tpl-pat-repeat-03', 'psl-p4-pat-repeated', {
    ...
    constraints: {
      ...
      _compute: {
        fullCycles: 'Math.floor(total / cycleLength)',
        leftover: 'total % cycleLength',
        answer: 'Math.floor(total / cycleLength) + (total % cycleLength >= targetColourPos ? 1 : 0)',
      },
    },
  ...
  mt('psl-tpl-pat-growing-01', 'psl-p5-pat-growing', {
    ...
    constraints: {
      ...
      _compute: {
        term1: 'start',
        term2: 'start + step',
        term3: 'start + step + (step + growth)',
        term4: 'start + step + (step + growth) + (step + 2 * growth)',
        diff1: 'step',
        diff2: 'step + growth',
        diff3: 'step + 2 * growth',
        nextDiff: 'step + 3 * growth',
        answer: 'start + step + (step + growth) + (step + 2 * growth) + (step + 3 * growth)',
      },
      answer: { min: 15 },
    },
  ...
  mt('psl-tpl-pat-growing-03', 'psl-p5-pat-growing', {
    ...
    constraints: {
      ...
      _compute: {
        ...
        term3: 'start + step + (step + growth)',
        term4: 'start + step + (step + growth) + (step + 2 * growth)',
        answer: 'start + (n - 1) * step + growth * (n - 2) * (n - 1) / 2',
      },
      answer: { min: 20 },
    },
  ...
  mt('psl-tpl-pat-multiply-01', 'psl-p5-pat-multiply', {
    ...
    constraints: {
      ...
      _compute: {
        term1: 'start',
        term2: 'start * multiplier',
        term3: 'start * multiplier ** 2',
        term4: 'start * multiplier ** 3',
