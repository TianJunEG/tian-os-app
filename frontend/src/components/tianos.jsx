...
// Tian OS design kit — unified brand palette (warm teal + ivory).
// Deep teal + warm coral CTAs, Fraunces serif headlines, Manrope body, light glassmorphism.
export const GOLD = '#D4A935';
export const GOLD_SOFT = 'rgba(212,169,53,0.35)';
export const CORAL = '#F28C7A';
export const CORAL_GLOW = 'rgba(242,140,122,0.45)';
...
export const glassPanel = (glow = false) => ({
  ...
  border: `1px solid ${glow ? 'rgba(15,76,92,0.18)' : 'rgba(15,76,92,0.08)'}`,
  borderRadius: 18,
  backdropFilter: 'blur(20px)',
  boxShadow: glow ? '0 24px 50px rgba(0,0,0,0.06), 0 0 40px rgba(15,76,92,0.06)' : '0 24px 50px rgba(0,0,0,0.04)',
});
...
export function Wordmark({ size = 30, onDark = false }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
      <span style={{ width: size, height: size, borderRadius: size * 0.3, display: 'grid', placeItems: 'center', fontFamily: SANS, fontWeight: 800, color: '#fff', fontSize: size * 0.56, background: TEAL, boxShadow: '0 4px 12px rgba(15,76,92,0.2)' }}>E</span>
...
export function Avatar({ label, size = 96, onDark = false }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
      <div style={{ width: size, height: size, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%, rgba(167,216,240,0.5), rgba(15,76,92,0.85))', border: '3px solid rgba(167,216,240,0.3)', boxShadow: '0 16px 40px rgba(0,0,0,0.1), 0 0 24px rgba(167,216,240,0.2)', display: 'grid', placeItems: 'center', fontFamily: SANS, fontWeight: 700, color: IVORY, fontSize: size * 0.34 }}>{label[0]}</div>
...
export function TrustRing({ nodes, onDark = false }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 540, aspectRatio: '1 / 1', margin: '0 auto' }}>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: '30%', aspectRatio: '1/1', borderRadius: '50%', background: `radial-gradient(circle at 30% 30%, ${SKY} 0%, ${TEAL} 55%, ${TEAL_DARK} 100%)`, display: 'grid', placeItems: 'center', fontFamily: SANS, fontWeight: 700, fontSize: 'clamp(13px,2.4vw,22px)', color: IVORY, letterSpacing: '0.06em', boxShadow: `0 0 70px rgba(15,76,92,0.35), 0 30px 60px rgba(0,0,0,0.12), inset 0 0 30px rgba(167,216,240,0.3)`, animation: 'tianosPulse 4s ease-in-out infinite' }}>TIAN·OS</div>
      {ring.map((n) => (
