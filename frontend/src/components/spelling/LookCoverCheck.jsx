  ...
  return (
    <div>
      ...
      <div className="flex justify-center gap-2 mb-8">
        {steps.map((s) => (
          <span key={s} className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${phase === s ? 'bg-emerald text-white' : 'bg-gray-100 text-gray-500'}`}>
            {s}
          </span>
        ))}
      </div>
      ...
      {phase === 'look' && (
        <div className="text-center">
          ...
          <button onClick={() => speakOnce(current.word, speakOptionsFor(lang))} className="text-emerald hover:bg-emerald-tint rounded-lg p-2 inline-flex items-center gap-1 text-sm">
            <Volume2 className="w-4 h-4" /> Hear it
          </button>
          {current.sentence && <p className="text-gray-500 mt-3">{current.sentence}</p>}
          <button onClick={() => setPhase('write')} className="mt-8 w-full py-3 bg-emerald text-white rounded-lg hover:bg-emerald-deep font-medium inline-flex items-center justify-center gap-2">
        ...
        hand ? (
          <div>
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button type="button" onClick={() => setPhase('check')} className="flex-1 py-3 bg-emerald text-white rounded-lg hover:bg-emerald-deep font-medium">
                Show answer
              </button>
            </div>
          </div>
          ...
          <form onSubmit={(e) => { e.preventDefault(); if (input.trim()) check(); }}>
            ...
            <input
              ...
              className="w-full text-center text-2xl py-4 border-2 border-gray-300 focus:border-emerald rounded-xl outline-none"
            />
            <div className="mt-6 flex gap-3">
              <button type="button" onClick={() => setPhase('look')} className="px-4 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 inline-flex items-center gap-2">
                <Eye className="w-5 h-5" /> Look again
              </button>
              <button type="submit" disabled={!input.trim()} className="flex-1 py-3 bg-emerald text-white rounded-lg hover:bg-emerald-deep disabled:opacity-40 font-medium">
                Check
              </button>
            </div>
          </form>
          ...
          {hand ? (
            <>
              ...
              {result == null ? (
                ...
                <button onClick={next} className="w-full py-3 bg-emerald text-white rounded-lg hover:bg-emerald-deep font-medium inline-flex items-center justify-center gap-2">
