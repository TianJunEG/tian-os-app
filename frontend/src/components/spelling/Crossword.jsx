        ...
        {list.map((entry) => (
          <li key={`${direction}-${entry.number}`}>
            <button
              ...
              className="text-left text-gray-600 hover:text-emerald"
            >
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="inline-flex rounded-lg border border-gray-200 overflow-hidden text-sm">
          {['across', 'down'].map((d) => (
            <button
              key={d}
              onClick={() => setDir(d)}
              className={`px-3 py-1.5 capitalize ${dir === d ? 'bg-emerald text-white' : 'bg-white text-gray-600'}`}
            >
              {d}
            </button>
          ))}
        ...
        <div className="flex gap-3 text-sm">
          ...
          <button onClick={reset} className="text-emerald hover:underline inline-flex items-center gap-1">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="overflow-x-auto">
                  ...
                  <div key={key(r, c)} className="relative w-8 h-8">
                    ...
                    <input
                      ...
                      className={`w-8 h-8 text-center text-sm font-bold uppercase outline-none rounded-sm ${
                        wrong ? 'bg-red-100 text-red-700' : right ? 'bg-green-100 text-green-700' : 'bg-white text-gray-800 focus:bg-emerald-tint'
                      }`}
                    />
                  </div>
                );
          ...
          <button
            onClick={check}
            className="mt-4 w-full py-2.5 bg-emerald text-white rounded-lg hover:bg-emerald-deep font-medium inline-flex items-center justify-center gap-2"
          >
