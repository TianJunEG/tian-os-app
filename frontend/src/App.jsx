...
import { GOLD, GOLD_SOFT, CORAL, CORAL_GLOW, TEAL, TEAL_DARK, IVORY, SKY, INK, INK_SOFT, BG, SANS, SERIF, Reveal, Eyebrow, Headline, GlassCard, Wordmark, TianOSKeyframes } from './components/tianos';
...
const LandingPage = () => (
  <div style={{ background: BG, color: INK, fontFamily: SANS, minHeight: '100vh', overflowX: 'hidden' }}>
    ...
    <header style={{ position: 'sticky', top: 0, zIndex: 40, backdropFilter: 'blur(12px)', background: 'rgba(255,248,234,0.85)', borderBottom: '1px solid rgba(15,76,92,0.08)' }}>
      ...
      <main>
        <section style={{ position: 'relative', overflow: 'hidden', background: `linear-gradient(to bottom, ${TEAL_DARK}, ${TEAL}, ${IVORY})` }}>
        <div style={{ position: 'absolute', right: '-6%', top: '-10%', width: 620, height: 620, borderRadius: '50%', background: 'radial-gradient(circle, rgba(167,216,240,0.3) 0%, transparent 60%)', filter: 'blur(40px)', pointerEvents: 'none', opacity: 0.5 }} />
        <div style={{ position: 'relative', maxWidth: 980, margin: '0 auto', padding: '120px 24px 130px', textAlign: 'center' }}>
          ...
          <Reveal delay={0.3} style={{ marginTop: 40, display: 'flex', flexWrap: 'wrap', gap: 16, justifyContent: 'center' }}>
            <Link to="/register?role=parent" style={{ display: 'inline-flex', alignItems: 'center', gap: 9, padding: '15px 30px', borderRadius: 999, background: CORAL, color: '#fff', fontWeight: 700, fontSize: 16, textDecoration: 'none', boxShadow: `0 20px 40px -12px ${CORAL_GLOW}, 0 0 40px ${CORAL_GLOW}` }}>Find a Tutor <ArrowRight size={18} /></Link>
            <Link to="/register?role=tutor" style={{ padding: '15px 30px', borderRadius: 999, background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(15,76,92,0.15)', color: TEAL, fontWeight: 600, fontSize: 16, textDecoration: 'none' }}>Become a Tutor</Link>
          </Reveal>
          <Reveal delay={0.4}>
            <Link to="/founder" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, marginTop: 28, color: CORAL, fontWeight: 600, fontSize: 15, textDecoration: 'none' }}>Read our founder story <ArrowRight size={15} /></Link>
          </Reveal>
        </div>
      </section>
          ...
          {LANDING_FEATURES.map((f, i) => (
            <Reveal key={f.title} delay={i * 0.1}>
              <GlassCard style={{ padding: 30, height: '100%' }}>
                <div style={{ width: 52, height: 52, borderRadius: 14, background: 'rgba(167,216,240,0.25)', border: '1px solid rgba(167,216,240,0.4)', display: 'grid', placeItems: 'center', marginBottom: 18 }}>
