  ...
  return (
    <div className="space-y-4">
        ...
        <table className="w-full text-xs sm:text-sm">
          <thead>
            <tr className="bg-ink-50">
              <th className="w-8 sm:w-10 px-1 sm:px-2 py-2 text-center text-xs font-medium text-ink-400">#</th>
              {columns.map((col) => (
                <th key={col} className="px-2 sm:px-3 py-2 text-left font-medium text-ink-600 whitespace-nowrap">
                  {col}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {guesses.map((row, rowIdx) => (
              <tr key={rowIdx} className="border-t border-ink-100">
                <td className="px-1 sm:px-2 py-1.5 text-center text-xs font-bold text-ink-400">
                  {rowIdx + 1}
                </td>
      ...
      {guesses.length < maxGuesses && (
        <button
          onClick={addRow}
          className="rounded-lg border border-dashed border-ink-300 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm text-ink-500 hover:border-ink-400 hover:text-ink-600"
        >
          + Add another guess
        </button>
      )}
