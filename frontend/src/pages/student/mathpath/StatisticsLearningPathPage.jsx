...
export default function StatisticsLearningPathPage() {
  const { user } = useAuth();
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/statistics/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const res = await mathpathAPI.statisticsSkillStates();
        const all = Array.isArray(res?.data?.records) ? res.data.records : [];
        const filtered = all.filter((r) => /ST0\d\d/.test(String(r.skillId || '')));
  ...
  const view = useMemo(() => buildStatisticsLearningPathView({ masteryRecords: records }), [records]);
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-5 px-3 py-5 sm:space-y-6 sm:px-4 sm:py-6">
      <PageHeader title="Statistics" subtitle="Pictograms, bar charts, line graphs and averages (P2–P6)." />
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 border-t border-ink-100 pt-3">
          <button type="button" onClick={() => navigate('/student/mathpath/statistics/diagnostic')} className="text-sm font-semibold text-emerald-deep hover:text-emerald-deep">
