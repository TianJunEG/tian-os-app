...
export default function TimePracticeSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await mathpathAPI.startTimePractice({ targetSkillId, questionCount: 6 });
        const data = res?.data || {};
  ...
  async function finish(allAnswers) {
    setSubmitting(true);
    try {
      const res = await mathpathAPI.submitTimePractice(session.practiceSessionId, buildSubmitPayload(allAnswers));
  ...
  if (error && !result) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Button className="mt-4" onClick={() => navigate('/student/mathpath/time')}>Back to Time</Button>
      </div>
    );
  }
  ...
  if (result) {
    const mascotMessage = result.accuracyPercentage >= 80
      ? `${result.accuracyPercentage}% — Time mastered — keep it up!`
      : result.accuracyPercentage >= 50
        ? `${result.accuracyPercentage}% — Good work on Time! Review the skills below to level up.`
        : `${result.accuracyPercentage}% — Every practice builds your time skills!`;
    return (
      <div className="mx-auto w-full max-w-2xl space-y-4 px-3 py-5 sm:space-y-5 sm:px-4 sm:py-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <Button onClick={() => navigate('/student/mathpath/time')}>Back to Skill Map</Button>
  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-4 px-3 py-5 sm:space-y-5 sm:px-4 sm:py-8">
      <PageHeader title="Time Practice" subtitle={session?.targetSkillId ? `Skill ${session.targetSkillId}` : ''} />
