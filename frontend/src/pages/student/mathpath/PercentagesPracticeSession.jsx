...
// Interactive Percentages practice session. Pulls a server-built question set
// (answers stripped) from /api/mathpath/percentages/practice/start, collects
...
// ── Component ───────────────────────────────────────────────────────────────
export default function PercentagesPracticeSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await mathpathAPI.startPercentagesPractice({ targetSkillId, questionCount: 6 });
        const data = res?.data || {};
  ...
  async function finish(allAnswers) {
    setSubmitting(true);
    try {
      const res = await mathpathAPI.submitPercentagesPractice(session.practiceSessionId, buildSubmitPayload(allAnswers));
  ...
  if (error && !result) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Button className="mt-4" onClick={() => navigate('/student/mathpath/percentages')}>Back to Percentages</Button>
      </div>
    );
  }

  if (result) {
    const taliaMessage = result.accuracyPercentage >= 80
      ? `${result.accuracyPercentage}% — outstanding work on percentages!`
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
        ...
        <MascotBubble name="talia" message={taliaMessage} size="sm" className="mb-2" />
        <Card className="p-5">
        ...
        <div className="flex gap-3">
          <Button onClick={() => navigate('/student/mathpath/percentages')}>Back to Skill Map</Button>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
      <PageHeader title="Percentages Practice" subtitle={session?.targetSkillId ? `Skill ${session.targetSkillId}` : ''} />
