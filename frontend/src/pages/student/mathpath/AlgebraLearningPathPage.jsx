...
export default function AlgebraLearningPathPage() {
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/algebra/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const res = await mathpathAPI.algebraSkillStates();
        const all = Array.isArray(res?.data?.records) ? res.data.records : [];
        const filtered = all.filter((r) => /AL0\d\d/.test(String(r.skillId || '')));
  ...
  const view = useMemo(() => buildAlgebraLearningPathView({ masteryRecords: records }), [records]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6">
      <PageHeader title="Algebra" subtitle="Patterns, unknowns, linear equations and algebraic manipulation (P4–P6)." />
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 border-t border-ink-100 pt-3">
          <button type="button" onClick={() => navigate('/student/mathpath/algebra/diagnostic')} className="text-sm font-semibold text-emerald-deep hover:text-emerald-deep">
