...
// Interactive Ratio & Rate practice session. Pulls a server-built question set
// (answers stripped) from /api/mathpath/ratio-rate/practice/start, collects
...
// ── Component ───────────────────────────────────────────────────────────────
export default function RatioRatePracticeSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await mathpathAPI.startRatioRatePractice({ targetSkillId, questionCount: 6 });
        const data = res?.data || {};
  ...
  async function finish(allAnswers) {
    setSubmitting(true);
    try {
      const res = await mathpathAPI.submitRatioRatePractice(session.practiceSessionId, buildSubmitPayload(allAnswers));
  ...
  if (error && !result) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Button className="mt-4" onClick={() => navigate('/student/mathpath/ratio-rate')}>Back to Ratio &amp; Rate</Button>
      </div>
    );
  }
  ...
  if (result) {
    const kyloMessage = result.accuracyPercentage >= 80
      ? `${result.accuracyPercentage}% — outstanding work on ratio & rate!`
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
        ...
        <div className="flex gap-3">
          <Button onClick={() => navigate('/student/mathpath/ratio-rate')}>Back to Skill Map</Button>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
      <PageHeader title="Ratio & Rate Practice" subtitle={session?.targetSkillId ? `Skill ${session.targetSkillId}` : ''} />
