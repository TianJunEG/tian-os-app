...
export default function OperationsPracticeSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await mathpathAPI.startOperationsPractice({ targetSkillId, questionCount: 6 });
        const data = res?.data || {};
  ...
  async function finish(allAnswers) {
    setSubmitting(true);
    try {
      const res = await mathpathAPI.submitOperationsPractice(session.practiceSessionId, buildSubmitPayload(allAnswers));
  ...
  if (error && !result) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Button className="mt-4" onClick={() => navigate('/student/mathpath/operations')}>Back to Operations</Button>
      </div>
    );
  }
  ...
  if (result) {
    const mascotMessage = result.accuracyPercentage >= 80
      ? `${result.accuracyPercentage}% — Operations mastered — keep it up!`
      : result.accuracyPercentage >= 50
        ? `${result.accuracyPercentage}% — Good work on Operations! Review the skills below to level up.`
        : `${result.accuracyPercentage}% — Every practice builds your operations skills!`;
    return (
      <div className="mx-auto w-full max-w-2xl space-y-4 px-3 py-5 sm:space-y-5 sm:px-4 sm:py-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <Button onClick={() => navigate('/student/mathpath/operations')}>Back to Skill Map</Button>
  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-4 px-3 py-5 sm:space-y-5 sm:px-4 sm:py-8">
      <PageHeader title="Operations Practice" subtitle={session?.targetSkillId ? `Skill ${session.targetSkillId}` : ''} />
