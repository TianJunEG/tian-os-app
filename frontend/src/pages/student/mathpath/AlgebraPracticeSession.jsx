...
export default function AlgebraPracticeSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await mathpathAPI.startAlgebraPractice({ targetSkillId, questionCount: 6 });
        const data = res?.data || {};
  ...
  async function finish(allAnswers) {
    setSubmitting(true);
    try {
      const res = await mathpathAPI.submitAlgebraPractice(session.practiceSessionId, buildSubmitPayload(allAnswers));
  ...
  if (error && !result) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Button className="mt-4" onClick={() => navigate('/student/mathpath/algebra')}>Back to Algebra</Button>
      </div>
    );
  }
  ...
  if (result) {
    const mascotMessage = result.accuracyPercentage >= 80
      ? `${result.accuracyPercentage}% — Algebra mastered — keep it up!`
      : result.accuracyPercentage >= 50
        ? `${result.accuracyPercentage}% — Good work on Algebra! Review the skills below to level up.`
        : `${result.accuracyPercentage}% — Every practice builds your algebra skills!`;
    return (
      <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
        ...
        <div className="flex gap-3">
          <Button onClick={() => navigate('/student/mathpath/algebra')}>Back to Skill Map</Button>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
      <PageHeader title="Algebra Practice" subtitle={session?.targetSkillId ? `Skill ${session.targetSkillId}` : ''} />
