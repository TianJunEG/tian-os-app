...
export default function CirclesLearningPathPage() {
  const { user } = useAuth();
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/circles/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const res = await mathpathAPI.circlesSkillStates();
        const all = Array.isArray(res?.data?.records) ? res.data.records : [];
        const filtered = all.filter((r) => /CI0\d\d/.test(String(r.skillId || '')));
  ...
  const view = useMemo(() => buildCirclesLearningPathView({ masteryRecords: records }), [records]);
