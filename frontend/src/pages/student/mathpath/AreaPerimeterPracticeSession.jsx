...
export default function AreaPerimeterPracticeSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await mathpathAPI.startAreaPerimeterPractice({ targetSkillId, questionCount: 6 });
        const data = res?.data || {};
  ...
  async function finish(allAnswers) {
    setSubmitting(true);
    try {
      const res = await mathpathAPI.submitAreaPerimeterPractice(session.practiceSessionId, buildSubmitPayload(allAnswers));
  ...
  if (error && !result) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Button className="mt-4" onClick={() => navigate('/student/mathpath/area-perimeter')}>Back to Area & Perimeter</Button>
      </div>
    );
  }
  ...
  if (result) {
    const mascotMessage = result.accuracyPercentage >= 80
      ? `${result.accuracyPercentage}% — Area & Perimeter mastered — keep it up!`
      : result.accuracyPercentage >= 50
        ? `${result.accuracyPercentage}% — Good work on Area & Perimeter! Review the skills below to level up.`
        : `${result.accuracyPercentage}% — Every practice builds your area & perimeter skills!`;
    return (
      <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
        <PageHeader title="Practice complete" subtitle={`You scored ${result.correct}/${result.total} (${result.accuracyPercentage}%).`} />
        <MascotBubble name="kylo" message={mascotMessage} size="sm" className="mb-2" />
        <Card className="p-5">
        ...
        <div className="flex gap-3">
          <Button onClick={() => navigate('/student/mathpath/area-perimeter')}>Back to Skill Map</Button>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
      <PageHeader title="Area & Perimeter Practice" subtitle={session?.targetSkillId ? `Skill ${session.targetSkillId}` : ''} />
