...
export default function StatisticsPracticeSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await mathpathAPI.startStatisticsPractice({ targetSkillId, questionCount: 6 });
        const data = res?.data || {};
  ...
  async function finish(allAnswers) {
    setSubmitting(true);
    try {
      const res = await mathpathAPI.submitStatisticsPractice(session.practiceSessionId, buildSubmitPayload(allAnswers));
  ...
  if (error && !result) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Button className="mt-4" onClick={() => navigate('/student/mathpath/statistics')}>Back to Statistics</Button>
      </div>
    );
  }
  ...
  if (result) {
    const mascotMessage = result.accuracyPercentage >= 80
      ? `${result.accuracyPercentage}% — Statistics mastered — keep it up!`
      : result.accuracyPercentage >= 50
        ? `${result.accuracyPercentage}% — Good work on Statistics! Review the skills below to level up.`
        : `${result.accuracyPercentage}% — Every practice builds your statistics skills!`;
    return (
      <div className="mx-auto w-full max-w-2xl space-y-4 px-3 py-5 sm:space-y-5 sm:px-4 sm:py-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <Button onClick={() => navigate('/student/mathpath/statistics')}>Back to Skill Map</Button>
  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-4 px-3 py-5 sm:space-y-5 sm:px-4 sm:py-8">
      <PageHeader title="Statistics Practice" subtitle={session?.targetSkillId ? `Skill ${session.targetSkillId}` : ''} />
