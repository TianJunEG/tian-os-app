...
export default function NumberSenseLearningPathPage() {
  const { user } = useAuth();
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/number-sense/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const res = await mathpathAPI.numberSenseSkillStates();
        const all = Array.isArray(res?.data?.records) ? res.data.records : [];
        const filtered = all.filter((r) => /NS0\d\d/.test(String(r.skillId || '')));
  ...
  const view = useMemo(() => buildNumberSenseLearningPathView({ masteryRecords: records }), [records]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6">
      <PageHeader title="Number Sense" subtitle="Whole numbers, place value, estimation and rounding (P1–P5)." />
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 border-t border-ink-100 pt-3">
          <button type="button" onClick={() => navigate('/student/mathpath/number-sense/diagnostic')} className="text-sm font-semibold text-emerald-deep hover:text-emerald-deep">
