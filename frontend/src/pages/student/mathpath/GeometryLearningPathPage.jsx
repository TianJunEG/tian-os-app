...
export default function GeometryLearningPathPage() {
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/geometry/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const res = await mathpathAPI.geometrySkillStates();
        const all = Array.isArray(res?.data?.records) ? res.data.records : [];
        const filtered = all.filter((r) => /GE0\d\d/.test(String(r.skillId || '')));
  ...
  const view = useMemo(() => buildGeometryLearningPathView({ masteryRecords: records }), [records]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6">
      <PageHeader title="Geometry" subtitle="Angles, triangles, quadrilaterals, symmetry and nets (P3–P6)." />
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 border-t border-ink-100 pt-3">
          <button type="button" onClick={() => navigate('/student/mathpath/geometry/diagnostic')} className="text-sm font-semibold text-emerald-deep hover:text-emerald-deep">
