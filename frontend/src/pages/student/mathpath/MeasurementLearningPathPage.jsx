...
export default function MeasurementLearningPathPage() {
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/measurement/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const res = await mathpathAPI.measurementSkillStates();
        const all = Array.isArray(res?.data?.records) ? res.data.records : [];
        const filtered = all.filter((r) => /ME0\d\d/.test(String(r.skillId || '')));
  ...
  const view = useMemo(() => buildMeasurementLearningPathView({ masteryRecords: records }), [records]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6">
      <PageHeader title="Measurement" subtitle="Length, mass, volume and converting between units (P1–P5)." />
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 border-t border-ink-100 pt-3">
          <button type="button" onClick={() => navigate('/student/mathpath/measurement/diagnostic')} className="text-sm font-semibold text-emerald-deep hover:text-emerald-deep">
