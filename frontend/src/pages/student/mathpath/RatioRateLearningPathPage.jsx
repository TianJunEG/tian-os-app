...
// Student-facing Ratio & Rate skill map. Presentational only: all status /
// prerequisite-locking / recommended-next logic lives in the pure
// buildRatioRateLearningPathView() model (shared + unit-tested). Practice launch
// is wired via the practise button; fluency/diagnostic/assessment not yet linked.
...
export default function RatioRateLearningPathPage() {
  const { user } = useAuth();
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/ratio-rate/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const statesRes = await mathpathAPI.ratioRateSkillStates();
        const all = Array.isArray(statesRes?.data?.records) ? statesRes.data.records : [];
        // Defensive: keep only R0-coded records in case the endpoint widens later.
        const ratioRateRecords = all.filter((r) => /^R0\d\d$/.test(String(r.skillId || '')));
        if (active) {
          setRecords(ratioRateRecords);
        }
      } catch {
  ...
  const view = useMemo(() => buildRatioRateLearningPathView({ masteryRecords: records }), [records]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6">
      <PageHeader title="Ratio & Rate" subtitle="Equivalent ratios, dividing in a ratio, speed and direct proportion (P5–P6)." />
