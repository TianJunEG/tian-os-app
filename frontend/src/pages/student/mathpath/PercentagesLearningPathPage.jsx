...
// Student-facing Percentages skill map. Presentational only: all status /
// prerequisite-locking / recommended-next logic lives in the pure
// buildPercentagesLearningPathView() model (shared + unit-tested). Practice
// launch is wired via the Practise button; fluency and assessment are
// decimals-only features and are omitted here.
...
export default function PercentagesLearningPathPage() {
  const { user } = useAuth();
  ...
  const startPractice = (skillId) => navigate(`/student/mathpath/percentages/practice?skill=${skillId}`);

  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const statesRes = await mathpathAPI.percentagesSkillStates();
        const all = Array.isArray(statesRes?.data?.records) ? statesRes.data.records : [];
        // Defensive: keep only P0-coded records in case the endpoint widens later.
        const percentageRecords = all.filter((r) => /^P0\d\d$/.test(String(r.skillId || '')));
        if (active) {
          setRecords(percentageRecords);
        }
      } catch {
  ...
  const view = useMemo(() => buildPercentagesLearningPathView({ masteryRecords: records }), [records]);
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-5 px-3 py-5 sm:space-y-6 sm:px-4 sm:py-6">
      <PageHeader title="Percentages" subtitle="Per hundred, conversions, discount, GST and interest (P5–P6)." />
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 border-t border-ink-100 pt-3">
          <button
            type="button"
            onClick={() => navigate('/student/mathpath/percentages/diagnostic')}
