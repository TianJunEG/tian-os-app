    ...
    <main className="mx-auto max-w-7xl pb-8">
      ...
      <Card className="mb-5 p-5">
        ...
        <form onSubmit={create} className="mt-4 grid gap-3 md:grid-cols-[1.4fr_1fr_1fr_1.2fr_auto]">
          <input className="rounded-xl border border-hairline px-3 py-2 text-sm" placeholder="Partner name" value={form.name} onChange={(event) => setForm((prev) => ({ ...prev, name: event.target.value }))} required />
          <select className="rounded-xl border border-hairline px-3 py-2 text-sm" value={form.type} onChange={(event) => setForm((prev) => ({ ...prev, type: event.target.value }))}>
            {TYPES.map((type) => <option key={type} value={type}>{label(type)}</option>)}
          </select>
          <select className="rounded-xl border border-hairline px-3 py-2 text-sm" value={form.status} onChange={(event) => setForm((prev) => ({ ...prev, status: event.target.value }))}>
            {STATUSES.map((status) => <option key={status} value={status}>{label(status)}</option>)}
          </select>
          <input className="rounded-xl border border-hairline px-3 py-2 text-sm" placeholder="Contact email" value={form.contactEmail} onChange={(event) => setForm((prev) => ({ ...prev, contactEmail: event.target.value }))} />
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="truncate font-display text-lg font-semibold text-navy-700">{partner.name}</h2>
                  </div>
