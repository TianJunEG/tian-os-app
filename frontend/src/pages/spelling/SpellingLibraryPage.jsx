        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          >
            ...
            <input
              ...
              className="w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-lg focus:border-emerald outline-none"
            />
          </form>
          <select value={level} onChange={(e) => setLevel(e.target.value)} className="px-3 py-2.5 border border-gray-300 rounded-lg bg-white focus:border-emerald outline-none">
            {LEVELS.map(([v, label]) => (
            ...
            {lists.map((list) => (
              <div key={list._id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex flex-col">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
                  <span className="px-2 py-0.5 bg-emerald-tint text-emerald-deep rounded">{LEVELS.find(([v]) => v === list.level)?.[1] || 'Other'}</span>
                ...
                <div className="mt-auto flex gap-2">
                  ...
                  <button onClick={() => copy(list._id)} disabled={copyingId === list._id} className="flex-1 py-2 text-sm bg-emerald text-white rounded-lg hover:bg-emerald-deep disabled:opacity-50 inline-flex items-center justify-center gap-1">
