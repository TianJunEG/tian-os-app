            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              {ACTIVITIES.map(([key, label]) => (
                <button
                  ...
                  className="py-3 px-2 bg-emerald text-white rounded-lg hover:bg-emerald-deep text-sm font-medium"
                >
                  {label}
                </button>
              ))}
              ...
              <ul className="divide-y divide-gray-50">
                {words.map((w) => (
                  <li key={w.word} className="flex items-center gap-3 py-2.5">
                    <button onClick={() => speakOnce(w.word)} className="p-1.5 text-emerald hover:bg-emerald-tint rounded" aria-label="Hear word">
