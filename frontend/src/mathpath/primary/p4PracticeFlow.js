...
// P4 Practice Flow
// ---------------------------------------------------------------------------
// Mirrors p3PracticeFlow.js but uses the P4 orchestrator and answer checker.
// Covers all 7 P4 domains (Whole Numbers, Factors & Multiples, Four Operations,
// Fractions, Decimals, Word Problems, Statistics) — 22 skills total.
//
// Returns the same session shape so PracticeSession.jsx can use any flow
...
// Question normalizer
// ---------------------------------------------------------------------------
// P4 generators return:   { answer: 5, answerType: 'number', options: [...], prompt, solutionText, ... }
// PracticeSession expects: { answer: { display, type, value }, type, choices, stem/prompt, solutionSteps, ... }

function normalizeP4Question(raw) {
  const normalized = { ...raw };

  // Curated questions may already have string answers
  const rawAnswer = raw._rawAnswer ?? raw.answer;
  const rawAnswerType = raw._rawAnswerType ?? raw.answerType;

  if (rawAnswerType === 'number' || rawAnswerType === 'numeric' || rawAnswerType === 'decimal'
      || typeof rawAnswer === 'number') {
    normalized.answer = {
      display: String(rawAnswer),
      type: 'whole',
      value: Number(rawAnswer) || rawAnswer,
    };
  } else if (rawAnswerType === 'money') {
    normalized.answer = {
      display: String(rawAnswer),
      type: 'text',
      value: rawAnswer,
    };
  } else if (rawAnswerType === 'choice' || rawAnswerType === 'exact') {
    normalized.answer = {
      display: String(rawAnswer),
      type: 'text',
      value: rawAnswer,
    ...
    if (raw.options || raw.choices) {
      normalized.type = 'mcq';
      normalized.choices = raw.options || raw.choices || [];
    }
  } else if (rawAnswerType === 'fraction') {
    normalized.answer = {
      display: String(rawAnswer),
      type: 'text',
      value: rawAnswer,
  ...
  // MCQ type detection for curated questions
  if (!normalized.type && raw.type === 'mcq' && raw.options) {
    normalized.type = 'mcq';
  ...
  normalized._rawAnswer = rawAnswer;
  normalized._rawAnswerType = rawAnswerType;
...
const P4_PREFIXES = ['P4-WN', 'P4-FM', 'P4-FO', 'P4-FR', 'P4-DEC', 'P4-WP', 'P4-STAT'];
...
export function isP4Domain(domainId) {
  ...
  return d.startsWith('p4-');
}

// ---------------------------------------------------------------------------
// Start P4 practice flow
// ---------------------------------------------------------------------------
...
export function startP4PracticeFlow(options = {}) {
  ...
  if (!targetSkillId) {
    ...
    targetSkillId = allSkills[0] || 'P4-WN-01';
  }
  ...
  const rawQuestions = generateQuestionSet(targetSkillId, effectiveSessionLength, {
    ...
    // For mastery checks and diagnostics, prefer curated exam questions
    preferCurated: resolvedSessionType === 'mastery_check' || resolvedSessionType === 'diagnostic',
  });
...
export function submitP4PracticeAttempt(options = {}) {
  ...
  const results = responses.map((response) => {
    ...
    return {
      ...
      feedback: buildP4PracticeFeedback({ correct: answerCheck.correct, fluencyFlag }),
