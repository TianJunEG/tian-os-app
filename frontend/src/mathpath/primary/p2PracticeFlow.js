...
// P2 Practice Flow
// ---------------------------------------------------------------------------
// Mirrors fractionPracticeFlow.js but uses the P2 orchestrator to generate
// questions across all 9 P2 domains (Whole Numbers, Add/Sub, Mul/Div, Money, Fractions,
// Statistics, Time, Geometry, Word Problems).
//
// Returns the same session shape so PracticeSession.jsx can use either flow
// interchangeably based on skill ID prefix.
// ---------------------------------------------------------------------------

import { generateQuestionSet, getDomainForSkill, getAllSupportedSkillIds } from './p2Orchestrator.js';
import { checkP2Answer } from './p2AnswerChecker.js';
import {
...
const P2_PREFIXES = ['P2-WN', 'P2-AS', 'P2-MD', 'P2-MON', 'P2-FR', 'P2-ST', 'P2-TM', 'P2-GEO', 'P2-WP'];
const P2_SPECIAL_IDS = ['P2-DIAGNOSTIC'];

export function isP2SkillId(skillId) {
  if (!skillId) return false;
  if (P2_SPECIAL_IDS.includes(skillId)) return true;
  return P2_PREFIXES.some((prefix) => skillId.startsWith(prefix));
}

export function isP2Domain(domainId) {
  if (!domainId) return false;
  const d = domainId.toLowerCase();
  return d.startsWith('p2-') || d === 'wholenumbers' || d === 'addsub' || d === 'muldiv' || d === 'money' || d === 'fractions'
    || d === 'stat' || d === 'time' || d === 'geo' || d === 'wordprob';
}
...
export function startP2PracticeFlow(options = {}) {
  const {
    studentId,
    domainId = null,
  ...
  if (!targetSkillId) {
    ...
    targetSkillId = allSkills[0] || 'P2-WN-01';
  }

  const resolvedDomainId = domainId || getDomainForSkill(targetSkillId) || 'p2-wholenumbers';
  ...
  const practiceSessionId = makeId('p2_practice');
  ...
  P2_FLOW_STORE.set(practiceSessionId, flowSession);
...
export function submitP2PracticeAttempt(options = {}) {
  const {
    practiceSessionId,
    studentId,
    sessionType = null,
  ...
  const session = P2_FLOW_STORE.get(practiceSessionId);
...
// ---------------------------------------------------------------------------
// Wraps checkP2Answer to accept the same { studentAnswer, correctAnswer, acceptedAnswers }
// signature that PracticeSession uses for checkFractionAnswer.

export function checkP2AnswerForSession({ studentAnswer, correctAnswer, acceptedAnswers = [], question }) {
  // If we have a normalized question with _rawAnswer, use it
  if (question?._rawAnswer !== undefined) {
    return checkP2Answer({
      studentAnswer,
      question: {
