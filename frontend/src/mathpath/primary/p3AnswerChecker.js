...
// P3 Answer Checker
// ---------------------------------------------------------------------------
// Handles the three answer types produced by P3 question generators:
...
/**
 * Check a student's answer against a P3 question's correct answer.
 *
 * @param {object}  options
 * @param {string}  options.studentAnswer  — raw student input (always a string)
 * @param {object}  options.question       — the full P3 question object
 * @returns {{ correct: boolean, normalizedStudentAnswer: string|null, normalizedCorrectAnswer: string }}
 */
export function checkP3Answer({ studentAnswer, question }) {
...
// This adapter lets P3 questions use the same call pattern.

export function checkP3AnswerCompat({ studentAnswer, correctAnswer, acceptedAnswers = [], question }) {
  // If we have the full question object, use the proper checker
  if (question) {
    return checkP3Answer({ studentAnswer, question });
  }
...
export default { checkP3Answer, checkP3AnswerCompat };
