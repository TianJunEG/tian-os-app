...
// P1 Practice Flow
// ---------------------------------------------------------------------------
// Mirrors fractionPracticeFlow.js but uses the P1 orchestrator to generate
// questions across all 7 P1 domains (Numbers, Add/Sub, Money, Measurement,
// Geometry, Equal Groups, Data — 58 skills total).
//
// Returns the same session shape so PracticeSession.jsx can use either flow
// interchangeably based on skill ID prefix.
// ---------------------------------------------------------------------------

import { generateQuestionSet, getDomainForSkill, getAllSupportedSkillIds } from './p1Orchestrator.js';
import { checkP1Answer } from './p1AnswerChecker.js';
import {
...
// Question normalizer
// ---------------------------------------------------------------------------
// P1 generators return:   { answer: 5, answerType: 'number', options: [...], prompt, solutionText, ... }
// PracticeSession expects: { answer: { display: '5', type: 'whole' }, type: 'mcq'?, choices: [...], stem/prompt, solutionSteps, ... }

function normalizeP1Question(raw) {
...
const P1_PREFIXES = ['P1-NUM', 'P1-ADD', 'P1-MON', 'P1-MEA', 'P1-GEO', 'P1-EQG', 'P1-DAT'];

export function isP1SkillId(skillId) {
  if (!skillId) return false;
  return P1_PREFIXES.some((prefix) => skillId.startsWith(prefix));
}

export function isP1Domain(domainId) {
  if (!domainId) return false;
  const d = domainId.toLowerCase();
  return d.startsWith('p1-') || d === 'numbers' || d === 'addsub' || d === 'money'
    || d === 'measurement' || d === 'geometry' || d === 'equalgroups' || d === 'data';
}

// ---------------------------------------------------------------------------
// Start P1 practice flow
// ---------------------------------------------------------------------------

export function startP1PracticeFlow(options = {}) {
  const {
    studentId,
    domainId = null,
  ...
  if (!targetSkillId) {
    ...
    targetSkillId = allSkills[0] || 'P1-NUM-01';
  }

  const resolvedDomainId = domainId || getDomainForSkill(targetSkillId) || 'p1-numbers';
  ...
  const questions = rawQuestions.map(normalizeP1Question);

  // Build working session
  const practiceSessionId = makeId('p1_practice');
  ...
  P1_FLOW_STORE.set(practiceSessionId, flowSession);
...
function buildP1PracticeFeedback(result = {}) {
...
function computeP1FamilySummary(results = []) {
...
// ---------------------------------------------------------------------------
// Submit P1 practice attempt
// ---------------------------------------------------------------------------

export function submitP1PracticeAttempt(options = {}) {
  const {
    practiceSessionId,
    studentId,
    sessionType = null,
  ...
  const session = P1_FLOW_STORE.get(practiceSessionId);
  ...
  const results = responses.map((response) => {
    ...
    // Use the P1 answer checker with the raw answer info
    ...
    return {
      ...
      feedback: buildP1PracticeFeedback({ correct: answerCheck.correct, fluencyFlag }),
  ...
  const familyFluencySummary = computeP1FamilySummary(scoredResults);
...
// ---------------------------------------------------------------------------
// Wraps checkP1Answer to accept the same { studentAnswer, correctAnswer, acceptedAnswers }
// signature that PracticeSession uses for checkFractionAnswer.

export function checkP1AnswerForSession({ studentAnswer, correctAnswer, acceptedAnswers = [], question }) {
  // If we have a normalized question with _rawAnswer, use it
  if (question?._rawAnswer !== undefined) {
    return checkP1Answer({
      studentAnswer,
      question: {
