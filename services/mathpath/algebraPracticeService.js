...
import { selectNextAlgebraPracticeTarget } from '../../shared/mathpath/algebra/AlgebraPracticeEngine.js';
...
export function buildAlgebraPracticeSession({
  ...
  if (!skillId || !getSkill(skillId)) {
    skillId = selectNextAlgebraPracticeTarget({ masteredSkillIds, weakSkillIds }).skillId;
  }
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown algebra skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
...
export function scoreAlgebraSubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses.filter((r) => r && r.questionId != null).map((r) => {
    ...
    const verdict = checkAlgebraAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
    return {
...
export default { buildAlgebraPracticeSession, toClientQuestions, scoreAlgebraSubmission, DOMAIN_ID };
