...
import { selectNextStatisticsPracticeTarget } from '../../shared/mathpath/statistics/StatisticsPracticeEngine.js';
...
export function buildStatisticsPracticeSession({
  ...
  if (!skillId || !getSkill(skillId)) {
    skillId = selectNextStatisticsPracticeTarget({ masteredSkillIds, weakSkillIds }).skillId;
  }
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown statistics skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
...
export function scoreStatisticsSubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses.filter((r) => r && r.questionId != null).map((r) => {
    ...
    const verdict = checkStatisticsAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
    return {
...
export default { buildStatisticsPracticeSession, toClientQuestions, scoreStatisticsSubmission, DOMAIN_ID };
