...
// Pure Ratio fluency-drill service. A fluency drill is a short, speed-focused
...
// the ratio practice service uses ('ratio'), so fluency + practice unify.
...
export function buildRatioFluencyDrill({ skillId, count = 8 } = {}) {
  if (!getSkill(skillId)) {
  ...
  const raw = generateRatioRateQuestionSet({ skillId, count, mode: 'fluency', sessionSalt: Date.now().toString() });
...
export function scoreRatioFluencyDrill({ skillId, questions = [], responses = [] } = {}) {
