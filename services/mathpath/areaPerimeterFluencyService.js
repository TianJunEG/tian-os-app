...
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/areaPerimeter/AreaPerimeterQuestionFamilies.js';
...
const DOMAIN_ID = 'area_perimeter';

function getFluencyBenchmarks(skillId) {
  const skill = getSkill(skillId);
  return { targetAverageSeconds: skill?.fluency?.targetAverageSeconds ?? 20, targetAccuracy: skill?.fluency?.targetAccuracy ?? 80, consistencyWindow: skill?.fluency?.consistencyWindow ?? 5 };
}
...
export function buildAreaPerimeterFluencyDrill({ skillId, studentId = null, count = 8 } = {}) {
  ...
  const questions = generateAreaPerimeterQuestionSet({ skillId, count, mode: 'fluency', sessionSalt: Date.now().toString() });
...
export function scoreAreaPerimeterFluencyDrill({ drill, answers = [], timingsSeconds = [] } = {}) {
