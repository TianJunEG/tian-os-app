...
} from '../../shared/mathpath/decimals/decimalsRetentionEngine.js';

// Thin service that turns the pure decimals retention engine into runnable
// review drills, mirroring decimalsRetentionService's build/score split.

export function buildDecimalsRetentionReview({ skillId, previousQuestionFamilyIds = [], count = null } = {}) {
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown decimals skill: ${skillId}`);
    err.status = 400;
    throw err;
  }
  ...
  const raw = generateDecimalQuestionSet({ skillId, count: questionCount, mode: 'fluency', sessionSalt: Date.now().toString() });
  if (!raw.length) {
...
export function scoreDecimalsRetentionReview({ skillId, questions = [], responses = [], completedIntervalDays = [], lastIntervalDays = null, completedAt = new Date() } = {}) {
