...
export function buildMoneyPracticeSession({
  ...
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown money skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
  const raw = generateMoneyQuestionSet({ skillId, count: questionCount, mode, sessionSalt: Date.now().toString() });
...
export function scoreMoneySubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses.filter((r) => r && r.questionId != null).map((r) => {
    ...
    const verdict = checkMoneyAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
    return {
...
export default { buildMoneyPracticeSession, toClientQuestions, scoreMoneySubmission, DOMAIN_ID };
