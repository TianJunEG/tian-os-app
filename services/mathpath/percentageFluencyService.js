...
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/percentages/percentageQuestionFamilies.js';
import {
  generatePercentageQuestion,
  checkPercentageAnswer,
} from '../../shared/mathpath/percentages/percentageQuestionGenerator.js';

// Pure Percentage fluency-drill service. A fluency drill is a short, speed-focused
...
//
// DOMAIN_ID is the canonical 'percentage' (matches percentageDiagnosticDomain.js).
// NOTE: percentagePracticeService.js persists under the legacy 'percentages' id;
// fluency intentionally uses the canonical id per the parity spec.

const DOMAIN_ID = 'percentage';
...
export function buildPercentageFluencyDrill({ skillId, count = 8 } = {}) {
  if (!getSkill(skillId)) {
  ...
  const questions = Array.from({ length: count }, (_, i) => {
    ...
    const q = generatePercentageQuestion({ questionFamilyId: family.id, variant, mode: 'fluency' });
    return {
...
export function scorePercentageFluencyDrill({ skillId, questions = [], responses = [] } = {}) {
