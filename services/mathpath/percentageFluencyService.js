...
// the percentage practice service uses ('percentage'), so fluency + practice unify.
...
export function buildPercentageFluencyDrill({ skillId, count = 8 } = {}) {
  ...
  const raw = generatePercentageQuestionSet({ skillId, count, mode: 'fluency', sessionSalt: Date.now().toString() });
