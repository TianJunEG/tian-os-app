...
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/statistics/StatisticsQuestionFamilies.js';
import {
  generateStatisticsQuestionSet,
  checkStatisticsAnswer,
} from '../../shared/mathpath/statistics/StatisticsQuestionGenerator.js';

// Pure Statistics fluency-drill service. A fluency drill is a short, speed-focused
// set of questions on one skill, scored into a fluency band (bronze → platinum)
// from accuracy + time + consistency. Mirrors ratioFluencyService exactly —
// only the imports and function names differ.

const DOMAIN_ID = 'statistics';
...
export function getFluencyBenchmarks(family = {}) {
  ...
  const base = family.fluencyTargetSeconds || 25;
  return {
...
export function computeConsistencyScore(attempts = [], benchmarkSeconds = 25) {
...
export function classifyFluency(metrics = {}) {
  ...
  const benchmarks = metrics.benchmarks || { bronze: 45, silver: 35, gold: 25, platinum: 18 };
...
export function buildStatisticsFluencyDrill({ skillId, count = 8 } = {}) {
  if (!getSkill(skillId)) {
...
export function scoreStatisticsFluencyDrill({ skillId, questions = [], responses = [] } = {}) {
  ...
  const benchmarks = questions[0]
    ...
    : { bronze: 45, silver: 35, gold: 25, platinum: 18 };
