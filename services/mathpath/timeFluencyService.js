...
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/time/TimeQuestionFamilies.js';
import { generateTimeQuestionSet, checkTimeAnswer } from '../../shared/mathpath/time/TimeQuestionGenerator.js';

const DOMAIN_ID = 'time';

function getFluencyBenchmarks(skillId) {
  const skill = getSkill(skillId);
  return { targetAverageSeconds: skill?.fluency?.targetAverageSeconds ?? 15, targetAccuracy: skill?.fluency?.targetAccuracy ?? 80, consistencyWindow: skill?.fluency?.consistencyWindow ?? 5 };
}
...
export function buildTimeFluencyDrill({ skillId, studentId = null, count = 8 } = {}) {
  ...
  const questionSet = generateTimeQuestionSet({ skillId, count, mode: 'fluency' });
...
export function scoreTimeFluencyDrill({ drill, answers = [], timingsSeconds = [] } = {}) {
  if (!drill) throw new Error('drill required');
  const results = drill.questions.map((q, i) => {
    const given = answers[i] ?? null;
    const correct = given != null ? checkTimeAnswer({ question: q, answer: given }) : false;
