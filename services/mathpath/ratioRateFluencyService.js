import { getSkill } from '../../shared/mathpath/ratioRate/ratioRateSkillGraph.js';
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/ratioRate/ratioRateQuestionFamilies.js';
import {
  generateRatioRateQuestion,
  checkRatioRateAnswer,
} from '../../shared/mathpath/ratioRate/ratioRateQuestionGenerator.js';

// Pure Ratio & Rate fluency-drill service. A fluency drill is a short, speed-focused
...
// derivation mirror the Fractions/Decimals fluency engines so every domain rates
// fluency on the same scale.
//
// DOMAIN_ID is the canonical 'ratio' (matches ratioDiagnosticDomain.js).
// NOTE: ratioRatePracticeService.js persists under the legacy 'ratioRate' id;
// fluency intentionally uses the canonical id per the parity spec.

const DOMAIN_ID = 'ratio';
export const FLUENCY_DOMAIN_ID = DOMAIN_ID;
...
export function buildRatioRateFluencyDrill({ skillId, count = 8 } = {}) {
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown ratio skill: ${skillId}`);
    err.status = 400;
    throw err;
  }
  ...
  const questions = Array.from({ length: count }, (_, i) => {
    ...
    const variant = Math.floor(i / families.length);
    const q = generateRatioRateQuestion({ questionFamilyId: family.id, variant, mode: 'fluency' });
    return {
      questionId: `${family.id}__f${variant}`,
      skillId: q.skillId,
...
export function scoreRatioRateFluencyDrill({ skillId, questions = [], responses = [] } = {}) {
  ...
  const attempts = responses
    ...
    .map((r) => {
      ...
      const correct = checkRatioRateAnswer({ question: q, studentResponse: r.studentAnswer ?? r.answer }).correct;
