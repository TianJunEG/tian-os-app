...
export function buildGeometryPracticeSession({
  ...
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown geometry skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
  const raw = generateGeometryQuestionSet({ skillId, count: questionCount, mode, sessionSalt: Date.now().toString() });
...
export function scoreGeometrySubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses.filter((r) => r && r.questionId != null).map((r) => {
    ...
    const verdict = checkGeometryAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
    return {
...
export default { buildGeometryPracticeSession, toClientQuestions, scoreGeometrySubmission, DOMAIN_ID };
