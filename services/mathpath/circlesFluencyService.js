...
export function buildCirclesFluencyDrill({ skillId, studentId = null, count = 8 } = {}) {
...
export function scoreCirclesFluencyDrill({ drill, answers = [], timingsSeconds = [] } = {}) {
  if (!drill) throw new Error('drill required');
  const results = drill.questions.map((q, i) => {
    const given = answers[i] ?? null;
    const correct = given != null ? checkCirclesAnswer({ question: q, answer: given }) : false;
