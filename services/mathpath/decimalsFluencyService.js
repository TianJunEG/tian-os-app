...
// Pure Decimals fluency-drill service. A fluency drill is a short, speed-focused
...
export function buildDecimalsFluencyDrill({ skillId, count = 8 } = {}) {
  if (!getSkill(skillId)) {
  ...
  const questions = Array.from({ length: count }, (_, i) => {
    ...
    const q = generateDecimalQuestion({ questionFamilyId: family.id, variant, mode: 'fluency' });
    return {
...
export function scoreDecimalsFluencyDrill({ skillId, questions = [], responses = [] } = {}) {
