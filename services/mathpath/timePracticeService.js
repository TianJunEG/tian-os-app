...
export function buildTimePracticeSession({
  ...
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown time skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
  const raw = generateTimeQuestionSet({ skillId, count: questionCount, mode, sessionSalt: Date.now().toString() });
...
export function scoreTimeSubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses.filter((r) => r && r.questionId != null).map((r) => {
    ...
    const verdict = checkTimeAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
    return {
...
export default { buildTimePracticeSession, toClientQuestions, scoreTimeSubmission, DOMAIN_ID };
