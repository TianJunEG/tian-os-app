...
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/volume/VolumeQuestionFamilies.js';
import {
  generateVolumeQuestionSet,
  checkVolumeAnswer,
} from '../../shared/mathpath/volume/VolumeQuestionGenerator.js';

// Pure Volume fluency-drill service. A fluency drill is a short, speed-focused
...
// derivation mirror the Fractions / Decimals fluency engines so all domains
// rate fluency on the same scale.

const DOMAIN_ID = 'volume';
...
export function buildVolumeFluencyDrill({ skillId, count = 8 } = {}) {
  if (!getSkill(skillId)) {
...
export function scoreVolumeFluencyDrill({ skillId, questions = [], responses = [] } = {}) {
