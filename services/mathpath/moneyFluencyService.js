...
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/money/MoneyQuestionFamilies.js';
import { generateMoneyQuestionSet, checkMoneyAnswer } from '../../shared/mathpath/money/MoneyQuestionGenerator.js';

const DOMAIN_ID = 'money';

function getFluencyBenchmarks(skillId) {
  const skill = getSkill(skillId);
  return { targetAverageSeconds: skill?.fluency?.targetAverageSeconds ?? 12, targetAccuracy: skill?.fluency?.targetAccuracy ?? 80, consistencyWindow: skill?.fluency?.consistencyWindow ?? 5 };
}
...
export function buildMoneyFluencyDrill({ skillId, studentId = null, count = 8 } = {}) {
  ...
  const questionSet = generateMoneyQuestionSet({ skillId, count, mode: 'fluency' });
...
export function scoreMoneyFluencyDrill({ drill, answers = [], timingsSeconds = [] } = {}) {
