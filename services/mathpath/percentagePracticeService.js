...
export function buildPercentagePracticeSession({
  targetSkillId = null,
  masteredSkillIds = [],
  weakSkillIds = [],
  questionCount = 6,
  ...
  if (!skillId || !getSkill(skillId)) {
    skillId = selectNextPercentagePracticeTarget({ masteredSkillIds, weakSkillIds }).skillId;
  }
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown percentages skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
...
export function scorePercentageSubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses
    ...
    .map((r) => {
      ...
      const verdict = checkPercentageAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
      return {
...
export default { buildPercentagePracticeSession, toClientQuestions, scorePercentageSubmission, DOMAIN_ID };
