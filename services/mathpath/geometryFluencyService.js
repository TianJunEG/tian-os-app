...
} from '../../shared/mathpath/geometry/GeometryQuestionFamilies.js';
import {
  generateGeometryQuestionSet,
  checkGeometryAnswer,
} from '../../shared/mathpath/geometry/GeometryQuestionGenerator.js';

// Pure Geometry fluency-drill service. A fluency drill is a short, speed-focused
...
// derivation mirror the Fractions/Decimals fluency engines so every domain
// rates fluency on the same scale. Geometry questions are visual — we keep the
// full question shape the generator returns (answer object, choices, prompt
// payload) intact and only attach a per-question fluency target.

const DOMAIN_ID = 'geometry';
...
// Fluency-appropriate families first: mental-math eligible, then fastest target.
...
// included, visual payload intact) for the route to persist; the route strips
// answers for the client.
export function buildGeometryFluencyDrill({ skillId, count = 8 } = {}) {
  if (!getSkill(skillId)) {
  ...
  const questions = raw.map((q, index) => {
    ...
    return {
      ...
      fluencyTargetSeconds: family.fluencyTargetSeconds ?? families[0].fluencyTargetSeconds,
    };
  });
...
export function scoreGeometryFluencyDrill({ skillId, questions = [], responses = [] } = {}) {
