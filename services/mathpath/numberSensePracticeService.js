import {
  generateNumberSenseQuestionSet,
...
export function buildNumberSensePracticeSession({
  ...
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown number-sense skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
...
export function scoreNumberSenseSubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses.filter((r) => r && r.questionId != null).map((r) => {
    ...
    const verdict = checkNumberSenseAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
    return {
...
export default { buildNumberSensePracticeSession, toClientQuestions, scoreNumberSenseSubmission, DOMAIN_ID };
