...
import { getQuestionFamiliesBySkill } from '../../shared/mathpath/numberSense/NumberSenseQuestionFamilies.js';
...
const DOMAIN_ID = 'number_sense';

function getFluencyBenchmarks(skillId) {
  const skill = getSkill(skillId);
  return { targetAverageSeconds: skill?.fluency?.targetAverageSeconds ?? 10, targetAccuracy: skill?.fluency?.targetAccuracy ?? 80, consistencyWindow: skill?.fluency?.consistencyWindow ?? 5 };
}
...
export function buildNumberSenseFluencyDrill({ skillId, studentId = null, count = 8 } = {}) {
  ...
  const questionSet = generateNumberSenseQuestionSet({ skillId, count, mode: 'fluency' });
...
export function scoreNumberSenseFluencyDrill({ drill, answers = [], timingsSeconds = [] } = {}) {
