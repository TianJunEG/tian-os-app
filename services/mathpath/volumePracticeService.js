...
import { selectNextVolumePracticeTarget } from '../../shared/mathpath/volume/VolumePracticeEngine.js';
...
export function buildVolumePracticeSession({
  ...
  if (!skillId || !getSkill(skillId)) {
    skillId = selectNextVolumePracticeTarget({ masteredSkillIds, weakSkillIds }).skillId;
  }
  if (!getSkill(skillId)) {
    const err = new Error(`Unknown volume skill: ${targetSkillId}`);
    err.status = 400;
    throw err;
  }
...
export function scoreVolumeSubmission({ questions = [], responses = [] } = {}) {
  ...
  const results = responses.filter((r) => r && r.questionId != null).map((r) => {
    ...
    const verdict = checkVolumeAnswer({ question, studentResponse: r.studentAnswer ?? r.answer });
    return {
...
export default { buildVolumePracticeSession, toClientQuestions, scoreVolumeSubmission, DOMAIN_ID };
