import { numberSenseSkillGraph, getSkill, getPrerequisites } from './NumberSenseSkillGraph.js';
import { getQuestionFamiliesBySkill, getQuestionFamily } from './NumberSenseQuestionFamilies.js';
...
const ORDERED_SKILL_IDS = numberSenseSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('NS', '')) || 0; }
function skillIdFor(n) { return `NS${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `numberSensepractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextNumberSensePracticeTarget(studentState = {}) {
...
export function buildNumberSensePracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'NS001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextNumberSensePracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'NumberSense skill practice session',
...
export function resetNumberSensePracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const numberSensePracticeEngine = {
  buildNumberSensePracticeSession, selectNextNumberSensePracticeTarget, resetNumberSensePracticeState,
};
export default numberSensePracticeEngine;
