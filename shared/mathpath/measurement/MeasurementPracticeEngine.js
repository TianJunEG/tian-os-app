import { measurementSkillGraph, getSkill, getPrerequisites } from './MeasurementSkillGraph.js';
import { getQuestionFamiliesBySkill, getQuestionFamily } from './MeasurementQuestionFamilies.js';
...
const ORDERED_SKILL_IDS = measurementSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('ME', '')) || 0; }
function skillIdFor(n) { return `ME${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `measurementpractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextMeasurementPracticeTarget(studentState = {}) {
...
export function buildMeasurementPracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'ME001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextMeasurementPracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'Measurement skill practice session',
...
export function resetMeasurementPracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const measurementPracticeEngine = {
  buildMeasurementPracticeSession, selectNextMeasurementPracticeTarget, resetMeasurementPracticeState,
};
export default measurementPracticeEngine;
