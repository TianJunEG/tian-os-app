...
function skillIdNumber(skillId) {
  return Number(String(skillId).replace('F', '')) || 0;
}

function buildSessionId() {
  return `fracpractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}
...
function getSkillThreshold(skillId) {
  const strand = getSkill(skillId)?.strand || 'Foundation';
...
export function selectNextFractionPracticeTarget(studentState = {}) {
  ...
  if (retentionDueSkillIds.length) {
    ...
    skillId = (studentState.masteredSkillIds || [])
      ...
      .reduce((next, mastered) => {
        const n = skillIdNumber(mastered) + 1;
...
export function determineFractionPracticeAction(state = {}) {
  const {
    accuracy = 0,
...
export function updateFractionPracticeState(attempt = {}) {
  const {
    studentId,
    skillId,
    questionFamilyId,
  ...
  const nextRecommendedAction = determineFractionPracticeAction({
...
export function getFractionPracticeQueue(studentState = {}) {
  const state = {
  ...
  const primary = selectNextFractionPracticeTarget(state);
...
export function buildFractionPracticeSession(options = {}) {
  const {
    studentId,
    diagnosticResult = {},
    masteredSkillIds = [],
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || diagnosticResult.recommendedStartingSkillId || existing.currentSkillId || 'F001',
    attemptHistory,
  ...
  const queue = getFractionPracticeQueue(mergedState);
  const top = queue[0] || selectNextFractionPracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: sessionGoalByReason[top.reason] || 'Fraction skill practice session',
    workingExpected,
    reason: top.reason,
    createdAt: nowIso(),
  };
...
export function validateFractionPracticeEngine() {
  const studentId = 'practice_validation_student';
  const diagnosticResult = {
    recommendedStartingSkillId: 'F018',
    masteredSkillIds: ['F001', 'F002', 'F003'],
    weakSkillIds: ['F018', 'F010'],
    suspectedPrerequisiteGaps: ['F010'],
  };
  ...
  const firstTarget = selectNextFractionPracticeTarget(getStudentState(studentId));
  const weakPrioritized = firstTarget.skillId === 'F010';

  const slowCorrect = updateFractionPracticeState({
    studentId,
    skillId: 'F016',
  ...
  updateFractionPracticeState({ studentId, skillId: 'F010', questionFamilyId: 'QF_F010_001', correct: false, timeTaken: 20, confidence: 'unsure', workingUploaded: false, attemptNumber: 1 });
  updateFractionPracticeState({ studentId, skillId: 'F010', questionFamilyId: 'QF_F010_001', correct: false, timeTaken: 22, confidence: 'unsure', workingUploaded: false, attemptNumber: 2 });
  const repeatIncorrect = updateFractionPracticeState({ studentId, skillId: 'F010', questionFamilyId: 'QF_F010_001', correct: false, timeTaken: 21, confidence: 'unsure', workingUploaded: false, attemptNumber: 3 });
  ...
  state.retentionDueSkillIds = ['F005'];
  setStudentState(studentId, state);
  const retentionTarget = selectNextFractionPracticeTarget(getStudentState(studentId));
  const retentionPrioritized = retentionTarget.skillId === 'F005' && retentionTarget.reason === 'retentionReview';

  updateFractionPracticeState({ studentId, skillId: 'F003', questionFamilyId: 'QF_F003_002', correct: true, timeTaken: 20, confidence: 'confident', workingUploaded: false, attemptNumber: 1 });
  const workingPrompt1 = updateFractionPracticeState({ studentId, skillId: 'F003', questionFamilyId: 'QF_F003_002', correct: true, timeTaken: 20, confidence: 'confident', workingUploaded: false, attemptNumber: 2 });
  const workingRequiredPrompt = workingPrompt1.nextRecommendedAction === 'requireWorkingUpload';

  const mentalMath = updateFractionPracticeState({ studentId, skillId: 'F001', questionFamilyId: 'QF_F001_001', correct: true, timeTaken: 10, confidence: 'confident', workingUploaded: false, attemptNumber: 1 });
  const mentalMathNoWorkingPrompt = mentalMath.nextRecommendedAction !== 'requireWorkingUpload';

  const queue = getFractionPracticeQueue(getStudentState(studentId));
...
export const fractionPracticeEngine = {
  buildFractionPracticeSession,
  selectNextFractionPracticeTarget,
  updateFractionPracticeState,
  getFractionPracticeQueue,
  determineFractionPracticeAction,
  validateFractionPracticeEngine,
};

export default fractionPracticeEngine;
