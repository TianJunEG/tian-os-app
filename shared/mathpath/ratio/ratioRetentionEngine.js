import { getQuestionFamiliesBySkill, getQuestionFamily } from '../ratioRate/ratioRateQuestionFamilies.js';
import { getSkill } from '../ratioRate/ratioRateSkillGraph.js';

// ---------------------------------------------------------------------------
// Ratio retention engine.
//
// This is the pure, persistence-free retention model for the Ratio domain.
// It mirrors the retention model of the algebra retention engine
// (shared/mathpath/algebra/algebraRetentionEngine.js) exactly:
...
// Fallback only — ratio skills may carry their own retention.reviewDays.
 ...
 * (fluent/retained) — the ratio fluency service writes both.
 */
 ...
 * skill-state row plus an `asOf` clock.
 */
...
export function summariseRetention({ states = [], asOf = new Date() } = {}) {
  ...
  return {
    ...
    retentionHistory: rows
      ...
      .map(({ state }) => shape(state)),
    emptyState: rows.length ? null : 'Retention reviews will appear after a ratio skill becomes fluent.',
  };
}
...
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return {
    reviewId: `RET_RAT_${skillId}_${Date.now()}`,
    domainId: DOMAIN_ID,
    skillId,
    sameConcept: true,
 ...
 * MathPathStudentSkillState row after a completed review.
 */
 ...
 * Self-test.
 */
export function validateRatioRetentionEngine() {
  const skillId = 'R003';
  ...
  const review = generateRetentionReview({ skillId, previousQuestionFamilyIds: ['QF_R003_001'] });
  ...
  const checks = {
    ...
    reviewUsesDifferentFamilies: !review.questionFamilyIds.includes('QF_R003_001'),
...
export const ratioRetentionEngine = {
  DOMAIN_ID,
  RETENTION_STATES,
  FLUENT_BANDS,
  FLUENT_STATUSES,
...
export default ratioRetentionEngine;
