import { getQuestionFamiliesBySkill, getQuestionFamily } from './AlgebraQuestionFamilies.js';
import { getSkill } from './AlgebraSkillGraph.js';

// ---------------------------------------------------------------------------
// Algebra retention engine (PILOT for non-fractions retention).
//
// This is the pure, persistence-free retention model for the Algebra domain.
// It mirrors the retention model of the fractions reference engine
...
//      metadata in the skill graph (algebra ships [3, 7, 30, 90]); the
//      DEFAULT_REVIEW_INTERVALS_DAYS fallback is only used if a skill omits it.
//   4. The review/schedule id prefixes (ALG).
...
export const DOMAIN_ID = 'algebra';
...
// Matches algebraFluencyService bands (bronze|silver|gold|platinum) + the
...
// Fallback only — algebra skills carry their own retention.reviewDays.
 ...
 * (fluent/retained) — the algebra fluency service writes both.
 */
...
export function summariseRetention({ states = [], asOf = new Date() } = {}) {
  ...
  return {
    ...
    retentionHistory: rows
      ...
      .map(({ state }) => shape(state)),
    emptyState: rows.length ? null : 'Retention reviews will appear after an algebra skill becomes fluent.',
  };
}
...
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return {
    reviewId: `RET_ALG_${skillId}_${Date.now()}`,
    domainId: DOMAIN_ID,
    skillId,
    sameConcept: true,
...
/**
 * Self-test, mirrors validateFractionFluencyRetentionEngine.
 */
export function validateAlgebraRetentionEngine() {
  const skillId = 'AL003';
  ...
  const review = generateRetentionReview({ skillId, previousQuestionFamilyIds: ['QF_AL003_001'] });
  ...
  const checks = {
    ...
    reviewUsesDifferentFamilies: !review.questionFamilyIds.includes('QF_AL003_001'),
...
export const algebraRetentionEngine = {
  DOMAIN_ID,
  RETENTION_STATES,
  FLUENT_BANDS,
  FLUENT_STATUSES,
...
export default algebraRetentionEngine;
