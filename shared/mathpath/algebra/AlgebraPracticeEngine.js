import { algebraSkillGraph, getSkill, getPrerequisites } from './AlgebraSkillGraph.js';
...
const ORDERED_SKILL_IDS = algebraSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('AL', '')) || 0; }
function skillIdFor(n) { return `AL${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `algebrapractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextAlgebraPracticeTarget(studentState = {}) {
...
export function buildAlgebraPracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'AL001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextAlgebraPracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'Algebra skill practice session',
...
export function resetAlgebraPracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const algebraPracticeEngine = {
  buildAlgebraPracticeSession, selectNextAlgebraPracticeTarget, resetAlgebraPracticeState,
};
export default algebraPracticeEngine;
