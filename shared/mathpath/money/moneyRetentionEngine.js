import { getQuestionFamiliesBySkill } from './MoneyQuestionFamilies.js';
...
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return { reviewId: `RET_MON_${skillId}_${Date.now()}`, domainId: DOMAIN_ID, skillId, sameConcept: true, differentQuestions: families.length > 0, preventMemorisation: true, questionFamilyIds: pool.slice(0, 4).map((f) => f.id), recommendedQuestionCount: Math.min(5, Math.max(3, pool.length)) };
}

export function classifyReviewOutcome({ accuracy = 0, averageTimeSeconds = null, skillId = '' } = {}) {
  const acc = toNum(accuracy);
  const ts = toNum(getSkill(skillId)?.fluency?.targetAverageSeconds, 10);
...
export function validateMoneyRetentionEngine() {
  const skillId = 'MN001'; const fluentAt = '2026-01-01T00:00:00.000Z';
  ...
  const retained = classifyReviewOutcome({ accuracy: 95, averageTimeSeconds: 7, skillId });
  const forgotten = classifyReviewOutcome({ accuracy: 30, skillId });
  const completion = applyReviewCompletion({ skillId, accuracy: 95, averageTimeSeconds: 7, lastIntervalDays: 3 });
