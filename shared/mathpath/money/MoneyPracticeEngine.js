import { moneySkillGraph, getSkill, getPrerequisites } from './MoneySkillGraph.js';
import { getQuestionFamiliesBySkill, getQuestionFamily } from './MoneyQuestionFamilies.js';
...
const ORDERED_SKILL_IDS = moneySkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('MN', '')) || 0; }
function skillIdFor(n) { return `MN${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `moneypractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextMoneyPracticeTarget(studentState = {}) {
...
export function buildMoneyPracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'MN001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextMoneyPracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'Money skill practice session',
...
export function resetMoneyPracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const moneyPracticeEngine = {
  buildMoneyPracticeSession, selectNextMoneyPracticeTarget, resetMoneyPracticeState,
};
export default moneyPracticeEngine;
