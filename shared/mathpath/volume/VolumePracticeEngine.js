import { volumeSkillGraph, getSkill, getPrerequisites } from './VolumeSkillGraph.js';
...
const ORDERED_SKILL_IDS = volumeSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('VL', '')) || 0; }
function skillIdFor(n) { return `VL${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `volumepractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextVolumePracticeTarget(studentState = {}) {
...
export function buildVolumePracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'VL001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextVolumePracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'Volume skill practice session',
...
export function resetVolumePracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const volumePracticeEngine = {
  buildVolumePracticeSession, selectNextVolumePracticeTarget, resetVolumePracticeState,
};
export default volumePracticeEngine;
