import { getQuestionFamiliesBySkill, getQuestionFamily } from './VolumeQuestionFamilies.js';
import { getSkill } from './VolumeSkillGraph.js';

// ---------------------------------------------------------------------------
// Volume retention engine.
//
// This is the pure, persistence-free retention model for the Volume domain.
// It follows the Algebra retention pilot (shared/mathpath/algebra/
// algebraRetentionEngine.js) verbatim in spirit:
...
//
// What is DOMAIN-SPECIFIC here (and therefore what differs from algebra):
...
//      metadata in the skill graph (volume ships [3, 7, 30, 90]); the
//      DEFAULT_REVIEW_INTERVALS_DAYS fallback is only used if a skill omits it.
//   4. The review/schedule id prefixes (RET_VOL).
...
// Matches volumeFluencyService bands (bronze|silver|gold|platinum) + the
...
// Fallback only — volume skills carry their own retention.reviewDays.
 ...
 * (fluent/retained) — the volume fluency service writes both.
 */
...
export function summariseRetention({ states = [], asOf = new Date() } = {}) {
  ...
  return {
    ...
    retentionHistory: rows
      ...
      .map(({ state }) => shape(state)),
    emptyState: rows.length ? null : 'Retention reviews will appear after a volume skill becomes fluent.',
  };
}
...
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return {
    reviewId: `RET_VOL_${skillId}_${Date.now()}`,
    domainId: DOMAIN_ID,
    skillId,
    sameConcept: true,
 ...
 */
export function validateVolumeRetentionEngine() {
  const skillId = 'VL002';
  ...
  const review = generateRetentionReview({ skillId, previousQuestionFamilyIds: ['QF_VL002_001'] });
  ...
  const checks = {
    ...
    reviewUsesDifferentFamilies: !review.questionFamilyIds.includes('QF_VL002_001'),
...
export const volumeRetentionEngine = {
  DOMAIN_ID,
  RETENTION_STATES,
  FLUENT_BANDS,
  FLUENT_STATUSES,
...
export default volumeRetentionEngine;
