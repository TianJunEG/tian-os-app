import { statisticsSkillGraph, getSkill, getPrerequisites } from './StatisticsSkillGraph.js';
import { getQuestionFamiliesBySkill, getQuestionFamily } from './StatisticsQuestionFamilies.js';
...
const ORDERED_SKILL_IDS = statisticsSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('ST', '')) || 0; }
function skillIdFor(n) { return `ST${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `statisticspractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextStatisticsPracticeTarget(studentState = {}) {
...
export function buildStatisticsPracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'ST001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextStatisticsPracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'Statistics skill practice session',
...
export function resetStatisticsPracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const statisticsPracticeEngine = {
  buildStatisticsPracticeSession, selectNextStatisticsPracticeTarget, resetStatisticsPracticeState,
};
export default statisticsPracticeEngine;
