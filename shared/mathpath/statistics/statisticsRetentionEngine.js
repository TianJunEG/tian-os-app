import { getQuestionFamiliesBySkill } from './StatisticsQuestionFamilies.js';
import { getSkill } from './StatisticsSkillGraph.js';

// ---------------------------------------------------------------------------
// Statistics retention engine.
//
// Pure, persistence-free retention model for the Statistics domain.
// Mirrors ratioRetentionEngine and decimalsRetentionEngine exactly —
// only the imports, DOMAIN_ID, id prefixes, and validate probe differ.
// ---------------------------------------------------------------------------

export const DOMAIN_ID = 'statistics';
...
export function summariseRetention({ states = [], asOf = new Date() } = {}) {
  ...
  return {
    ...
    retentionHistory: rows
      ...
      .map(({ state }) => shape(state)),
    emptyState: rows.length ? null : 'Retention reviews will appear after a statistics skill becomes fluent.',
  };
}
...
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return {
    reviewId: `RET_STAT_${skillId}_${Date.now()}`,
    domainId: DOMAIN_ID,
    skillId,
    sameConcept: true,
...
export function classifyReviewOutcome({ accuracy = 0, averageTimeSeconds = null, skillId = '' } = {}) {
  ...
  const targetSeconds = toNum(getSkill(skillId)?.fluency?.targetAverageSeconds, 12);
...
export function validateStatisticsRetentionEngine() {
  const skillId = 'ST001';
  ...
  const checks = {
    ...
    reviewGeneratesQuestionFamilies: review.questionFamilyIds.length > 0,
...
export const statisticsRetentionEngine = {
  DOMAIN_ID,
  RETENTION_STATES,
  FLUENT_BANDS,
  FLUENT_STATUSES,
  ...
  validateStatisticsRetentionEngine,
};

export default statisticsRetentionEngine;
