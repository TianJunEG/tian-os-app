import { getQuestionFamiliesBySkill, getQuestionFamily } from '../percentages/percentageQuestionFamilies.js';
import { getSkill } from '../percentages/percentageSkillGraph.js';

// ---------------------------------------------------------------------------
// Percentage retention engine.
//
// This is the pure, persistence-free retention model for the Percentage domain.
// It mirrors the retention model of the algebra retention engine in spirit:
...
//
// What is DOMAIN-SPECIFIC here (and therefore what a future domain changes):
...
//      metadata in the skill graph; the DEFAULT_REVIEW_INTERVALS_DAYS fallback
//      is only used if a skill omits it.
//   4. The review/schedule id prefixes (PCT).
// Everything else — the scheduling math, the state machine, due/forgetting
// detection — is generic and should be copied verbatim by other domains.
// ---------------------------------------------------------------------------
...
// Matches percentageFluencyService bands (bronze|silver|gold|platinum) + the
...
// Fallback only — percentage skills carry their own retention.reviewDays.
 ...
 * (fluent/retained) — the percentage fluency service writes both.
 */
 ...
 * Mirrors algebra's buildRetentionScheduleFromFluency contract:
 ...
 * retention is due / at risk. Mirrors algebra detectForgetting().
 */
 ...
 * persisted skill-state rows. Same shape contract as algebra's
 * summariseRetention (upcomingReviews / overdueReviews / retentionHistory).
 */
export function summariseRetention({ states = [], asOf = new Date() } = {}) {
  ...
  return {
    ...
    retentionHistory: rows
      ...
      .map(({ state }) => shape(state)),
    emptyState: rows.length ? null : 'Retention reviews will appear after a percentage skill becomes fluent.',
  };
}
 ...
 * Mirrors algebra generateRetentionReview().
 */
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return {
    reviewId: `RET_PCT_${skillId}_${Date.now()}`,
    domainId: DOMAIN_ID,
    skillId,
    sameConcept: true,
 ...
 * retentionStatus. Mirrors the algebra route's retained/retention_risk gate.
 */
 ...
 */
export function validatePercentageRetentionEngine() {
  const skillId = 'P003';
  ...
  const checks = {
    ...
    reviewHasQuestionFamilies: review.questionFamilyIds.length > 0,
...
export const percentageRetentionEngine = {
  DOMAIN_ID,
  RETENTION_STATES,
  FLUENT_BANDS,
  FLUENT_STATUSES,
...
export default percentageRetentionEngine;
