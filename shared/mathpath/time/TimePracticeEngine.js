import { timeSkillGraph, getSkill, getPrerequisites } from './TimeSkillGraph.js';
import { getQuestionFamiliesBySkill, getQuestionFamily } from './TimeQuestionFamilies.js';
...
const ORDERED_SKILL_IDS = timeSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('TM', '')) || 0; }
function skillIdFor(n) { return `TM${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `timepractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextTimePracticeTarget(studentState = {}) {
...
export function buildTimePracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'TM001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextTimePracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'Time skill practice session',
...
export function resetTimePracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const timePracticeEngine = {
  buildTimePracticeSession, selectNextTimePracticeTarget, resetTimePracticeState,
};
export default timePracticeEngine;
