import { circlesSkillGraph, getSkill, getPrerequisites } from './CirclesSkillGraph.js';
...
const ORDERED_SKILL_IDS = circlesSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('CI', '')) || 0; }
function skillIdFor(n) { return `CI${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `circlespractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextCirclesPracticeTarget(studentState = {}) {
...
export function buildCirclesPracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'CI001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextCirclesPracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'Circles skill practice session',
...
export function resetCirclesPracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const circlesPracticeEngine = {
  buildCirclesPracticeSession, selectNextCirclesPracticeTarget, resetCirclesPracticeState,
};
export default circlesPracticeEngine;
