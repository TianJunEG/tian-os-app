import { getQuestionFamiliesBySkill } from './AreaPerimeterQuestionFamilies.js';
...
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return { reviewId: `RET_AP_${skillId}_${Date.now()}`, domainId: DOMAIN_ID, skillId, sameConcept: true, differentQuestions: families.length > 0, preventMemorisation: true, questionFamilyIds: pool.slice(0, 4).map((f) => f.id), recommendedQuestionCount: Math.min(5, Math.max(3, pool.length)) };
}
...
export function validateAreaPerimeterRetentionEngine() {
  const skillId = 'AP001'; const fluentAt = '2026-01-01T00:00:00.000Z';
