import { areaPerimeterSkillGraph, getSkill, getPrerequisites } from './AreaPerimeterSkillGraph.js';
import { getQuestionFamiliesBySkill, getQuestionFamily } from './AreaPerimeterQuestionFamilies.js';
...
const ORDERED_SKILL_IDS = areaPerimeterSkillGraph.skillIds;

function nowIso() { return new Date().toISOString(); }
function skillIdNumber(skillId) { return Number(String(skillId).replace('AP', '')) || 0; }
function skillIdFor(n) { return `AP${String(n).padStart(3, '0')}`; }
function buildSessionId() { return `areaPerimeterpractice_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
...
export function selectNextAreaPerimeterPracticeTarget(studentState = {}) {
...
export function buildAreaPerimeterPracticeSession(options = {}) {
  const {
    studentId, masteredSkillIds = [], weakSkillIds = [], currentSkillId = null,
  ...
  const mergedState = {
    ...
    currentSkillId: currentSkillId || existing.currentSkillId || 'AP001',
    attemptHistory, retentionDueSkillIds: existing.retentionDueSkillIds || [], familyHistory: existing.familyHistory || {},
  };
  const top = selectNextAreaPerimeterPracticeTarget(mergedState);
  ...
  const session = {
    ...
    sessionGoal: 'AreaPerimeter skill practice session',
...
export function resetAreaPerimeterPracticeState(studentId) { PRACTICE_STORE.delete(studentId); }

export const areaPerimeterPracticeEngine = {
  buildAreaPerimeterPracticeSession, selectNextAreaPerimeterPracticeTarget, resetAreaPerimeterPracticeState,
};
export default areaPerimeterPracticeEngine;
