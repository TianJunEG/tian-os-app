...
const BUILDERS = {
  ...
  [GE(1)]: (rng) => { const p = pick(rng, POLY); return { prompt: `How many sides does a ${p.name} have?`, answer: p.sides, tag: 'geo/orientation-changes-shape', steps: [`A ${p.name} has ${p.sides} straight sides.`], distractors: [p.sides + 1, p.sides - 1, p.sides + 2], diagram: { kind: 'polygon', sides: p.sides, labelMode: 'none' } }; },
  // GE002 — Properties: sides and vertices
  [GE(2)]: (rng) => { const p = pick(rng, POLY); const ask = pick(rng, ['sides', 'vertices']); return { prompt: `How many ${ask} does a ${p.name} have?`, answer: p.sides, tag: 'geo/side-vertex-count', steps: [`A ${p.name} has ${p.sides} ${ask} (a polygon has as many vertices as sides).`], distractors: [p.sides + 1, p.sides - 1, p.sides * 2], diagram: { kind: 'polygon', sides: p.sides, labelMode: 'none' } }; },
  ...
  // ── Word-problem families (_003): real-world context. Figure-describing items
  // carry the same diagram as their non-word sibling (the renderer already
  // supports these kinds); pure-fact / answer-leaking ones (lines, quad-naming,
  // compass, construction facts) intentionally stay diagram-less. ──────────────
  'GE001W': (rng) => { const p = pick(rng, POLY); const obj = pick(rng, ['road sign', 'tile', 'badge', 'window pane']); return { prompt: `A ${obj} is shaped like a ${p.name}. How many sides does a ${p.name} have?`, answer: p.sides, tag: 'geo/orientation-changes-shape', steps: [`A ${p.name} has ${p.sides} straight sides.`], distractors: [p.sides + 1, p.sides - 1, p.sides + 2], diagram: { kind: 'polygon', sides: p.sides, labelMode: 'none' } }; },
  'GE002W': (rng) => { const p = pick(rng, POLY); const ask = pick(rng, ['sides', 'vertices']); const obj = pick(rng, ['honeycomb cell', 'floor tile', 'garden bed', 'clock face']); return { prompt: `A ${obj} is shaped like a ${p.name}. How many ${ask} does it have?`, answer: p.sides, tag: 'geo/side-vertex-count', steps: [`A ${p.name} has ${p.sides} ${ask}.`], distractors: [p.sides + 1, p.sides - 1, p.sides * 2], diagram: { kind: 'polygon', sides: p.sides, labelMode: 'none' } }; },
  ...
  'GE005W': (rng) => { const x = pick(rng, [30, 45, 60, 75]); const comp = 90 - x; return { prompt: `A ramp makes an angle of ${x}° with the ground. What angle does the ramp make with the vertical wall above it?`, answer: comp, unit: '°', tag: 'geo/protractor-scale', steps: [`The ramp meets the wall at a corner, so ramp + wall angle = 90°.`, `90 − ${x} = ${comp}°.`], distractors: [x, 180 - x, comp + 10].filter((d) => d > 0 && d !== comp), diagram: { kind: 'angle', degrees: x } }; },
  'GE006W': (rng) => { const atPoint = rng() < 0.5; const total = atPoint ? 360 : 180; const known = atPoint ? rint(rng, 80, 250) : rint(rng, 20, 160); const a = total - known; return { prompt: atPoint ? `Angles at a point add up to 360°. One angle is ${known}°. Find the other angle.` : `Angles on a straight line add up to 180°. One angle is ${known}°. Find the other angle.`, answer: a, unit: '°', tag: 'geo/angle-relationship', steps: [`${total} − ${known} = ${a}°.`], distractors: [(atPoint ? 180 : 360) - known, known, a + 10].filter((d) => d > 0 && d !== a), diagram: { kind: 'angle', degrees: known, partner: a, total } }; },
  ...
  'GE008W': (rng) => { const a = rint(rng, 30, 80), b = rint(rng, 30, 80); const c = 180 - a - b; const obj = pick(rng, ['sign', 'sail', 'roof panel', 'garden frame']); return { prompt: `A triangular ${obj} is cut so that two angles of a triangle are ${a}° and ${b}°. Find the third angle.`, answer: c, unit: '°', tag: 'geo/angle-sum-wrong', steps: ['The angles of a triangle add up to 180°.', `180 − ${a} − ${b} = ${c}°.`], distractors: [360 - a - b, a + b, 180 - a].filter((d) => d > 0 && d !== c), diagram: { kind: 'triangle', angles: [a, b, c] } }; },
  'GE009W': (rng) => { const q = pick(rng, [{ n: 'square', d: '4 equal sides and 4 right angles' }, { n: 'rectangle', d: '4 right angles but not all four sides equal' }, { n: 'rhombus', d: '4 equal sides but no right angles' }, { n: 'parallelogram', d: 'two pairs of parallel sides, but no right angles and not all sides equal' }]); const obj = pick(rng, ['floor tile', 'picture frame', 'table top', 'field']); return { prompt: `A ${obj} has ${q.d}. What quadrilateral shape is it?`, answer: q.n, tag: 'geo/square-not-rectangle', steps: [`A ${q.n} has ${q.d}.`], choices: ['square', 'rectangle', 'rhombus', 'parallelogram'] }; },
  'GE010W': (rng) => { const a = rint(rng, 50, 130); const adj = 180 - a; const obj = pick(rng, ['picture frame', 'tabletop', 'tile', 'sign']); return { prompt: `A ${obj} is shaped like a parallelogram. One angle is ${a}°. Find the angle next to it (co-interior).`, answer: adj, unit: '°', tag: 'geo/quad-angle-relationship', steps: ['Adjacent angles in a parallelogram add up to 180°.', `180 − ${a} = ${adj}°.`], distractors: [a, 360 - a, adj + 10].filter((d) => d > 0 && d !== adj), diagram: { kind: 'parallelogram', angle: a } }; },
  'GE011W': (rng) => { const s = pick(rng, SYMMETRY); const obj = pick(rng, ['tile', 'badge', 'window', 'flag panel']); return { prompt: `A ${obj} is shaped like a ${s.name}. How many lines of symmetry does it have?`, answer: s.lines, tag: 'geo/diagonal-symmetry-miss', steps: [`A ${s.name} has ${s.lines} lines of symmetry.`], distractors: [s.lines + 1, s.lines - 1, s.lines + 2].filter((d) => d >= 0), diagram: { kind: 'symmetry', shape: s.name, lines: s.lines } }; },
  'GE012W': (rng) => { const k = rint(rng, 1, 6); const obj = pick(rng, ['butterfly', 'leaf', 'snowflake design', 'kite shape']); return { prompt: `A ${obj} has a vertical line of symmetry. One side shows a dot ${k} square(s) to the left of the line. How many squares to the right is its reflection?`, answer: k, unit: 'squares', tag: 'geo/reflect-not-translate', steps: ['A reflection is the same distance on the other side.', `${k} left → ${k} right.`], distractors: [k + 1, k * 2, k + 2], diagram: { kind: 'reflection', distance: k } }; },
  ...
  'GE017W': (rng) => { const s = rint(rng, 3, 20); const obj = pick(rng, ['square garden', 'square tile', 'square sandbox', 'square table top']); return { prompt: `A ${obj} has sides of ${s} cm. What is its perimeter?`, answer: 4 * s, unit: 'cm', tag: 'geo/perimeter-vs-area', steps: ['Perimeter of a square = 4 × side.', `4 × ${s} = ${4 * s} cm.`], distractors: [s * s, 2 * s, 4 * s + s].filter((d) => d !== 4 * s), diagram: { kind: 'square', side: s } }; },
  'GE018W': (rng) => { const l = rint(rng, 3, 18), w = rint(rng, 2, 12); const obj = pick(rng, ['carpet', 'field', 'wall panel', 'swimming pool']); return { prompt: `A ${obj} is ${l} cm long and ${w} cm wide. What is its area?`, answer: l * w, unit: 'cm²', tag: 'geo/area-add-sides', steps: ['Area = length × width.', `${l} × ${w} = ${l * w} cm².`], distractors: [2 * (l + w), l + w, l * w + l].filter((d) => d !== l * w), diagram: { kind: 'rectangle', l, w } }; },
