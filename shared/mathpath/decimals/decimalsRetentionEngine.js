import { getQuestionFamiliesBySkill } from './decimalsQuestionFamilies.js';
import { getSkill } from './decimalsSkillGraph.js';

// ---------------------------------------------------------------------------
// Decimals retention engine.
//
// Pure, persistence-free retention model for the Decimals domain.
// Mirrors ratioRetentionEngine and algebraRetentionEngine exactly —
...
export function summariseRetention({ states = [], asOf = new Date() } = {}) {
  ...
  return {
    ...
    retentionHistory: rows
      ...
      .map(({ state }) => shape(state)),
    emptyState: rows.length ? null : 'Retention reviews will appear after a decimals skill becomes fluent.',
  };
}
...
export function generateRetentionReview({ skillId = '', previousQuestionFamilyIds = [], difficulty = null } = {}) {
  ...
  return {
    reviewId: `RET_DEC_${skillId}_${Date.now()}`,
    domainId: DOMAIN_ID,
    skillId,
    sameConcept: true,
...
export function validateDecimalsRetentionEngine() {
  const skillId = 'D001';
...
export const decimalsRetentionEngine = {
  DOMAIN_ID,
  RETENTION_STATES,
  FLUENT_BANDS,
  FLUENT_STATUSES,
  ...
  validateDecimalsRetentionEngine,
};

export default decimalsRetentionEngine;
