...
import {
  ...
  validateVolumeRetentionEngine,
} from '../shared/mathpath/volume/volumeRetentionEngine.js';

const FLUENT_AT = '2026-01-01T00:00:00.000Z';

describe('Volume retention — scheduling', () => {
  it('reads review intervals from the skill graph (volume ships [3,7,30,90])', () => {
    expect(reviewIntervalsForSkill('VL002')).toEqual([3, 7, 30, 90]);
  });

  it('builds dated reviews from the fluent-at anchor using the skill intervals', () => {
    const schedule = buildSpacedReviewSchedule({ skillId: 'VL002', fluentAt: FLUENT_AT });
...
describe('Volume retention — mastery gate', () => {
  ...
  it('blocks scheduling for non-fluent skills, allows it for fluent ones', () => {
    const blocked = buildRetentionScheduleFromFluency({ skillId: 'VL002', fluencyLevel: 'bronze', fluentAt: FLUENT_AT });
    expect(blocked.shouldSchedule).toBe(false);
    expect(blocked.reviews).toHaveLength(0);

    const allowed = buildRetentionScheduleFromFluency({ skillId: 'VL002', fluencyLevel: 'gold', fluentAt: FLUENT_AT });
...
describe('Volume retention — due detection', () => {
  it('finds the earliest passed-but-uncompleted review', () => {
    const due = findNextDueReview({ skillId: 'VL002', fluentAt: FLUENT_AT, completedIntervalDays: [], asOf: '2026-01-05T00:00:00.000Z' });
    expect(due.intervalDays).toBe(3);
  });

  it('skips reviews already completed', () => {
    const due = findNextDueReview({ skillId: 'VL002', fluentAt: FLUENT_AT, completedIntervalDays: [3], asOf: '2026-01-09T00:00:00.000Z' });
    expect(due.intervalDays).toBe(7);
  });

  it('returns null before any review is due', () => {
    const due = findNextDueReview({ skillId: 'VL002', fluentAt: FLUENT_AT, completedIntervalDays: [], asOf: '2026-01-02T00:00:00.000Z' });
    expect(due).toBeNull();
  });

  it('marks a fluent skill as review_due once the first interval has passed', () => {
    const status = evaluateRetentionStatus({
      state: { skillId: 'VL002', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
  ...
  it('detectForgetting surfaces only skills whose retention is at risk', () => {
    const alerts = detectForgetting({
      states: [
        { skillId: 'VL002', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
        { skillId: 'VL003', fluentAt: '2026-01-07T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
      ],
      asOf: '2026-01-08T00:00:00.000Z',
    });
    const ids = alerts.map((a) => a.skillId);
    expect(ids).toContain('VL002');
    expect(ids).not.toContain('VL003');
  });
});

describe('Volume retention — summary shape', () => {
  it('buckets skill-states into upcoming / overdue / history', () => {
    const summary = summariseRetention({
      states: [
        // upcoming: fluent but not yet due
        { skillId: 'VL001', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled', nextReviewDate: '2026-02-01T00:00:00.000Z' },
        // overdue: due date passed
        { skillId: 'VL002', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
        // history: retained
        { skillId: 'VL004', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'retained', retentionStatus: 'retained' },
      ],
      asOf: '2026-01-15T00:00:00.000Z',
    });
    expect(summary.domainId).toBe('volume');
    expect(summary.upcomingReviews.map((r) => r.skillId)).toContain('VL001');
    expect(summary.overdueReviews.map((r) => r.skillId)).toContain('VL002');
    expect(summary.retentionHistory.map((r) => r.skillId)).toContain('VL004');
  });
...
describe('Volume retention — review generation', () => {
  it('produces a same-concept review using different question families', () => {
    const review = generateRetentionReview({ skillId: 'VL002', previousQuestionFamilyIds: ['QF_VL002_001'] });
    expect(review.skillId).toBe('VL002');
    expect(review.sameConcept).toBe(true);
    expect(review.questionFamilyIds).not.toContain('QF_VL002_001');
    expect(review.questionFamilyIds.length).toBeGreaterThan(0);
  });
});

describe('Volume retention — outcome classification + completion', () => {
  it('classifies retained / needs-review / forgotten by accuracy + speed', () => {
    expect(classifyReviewOutcome({ accuracy: 95, averageTimeSeconds: 6, skillId: 'VL002' }).retentionStatus).toBe(RETENTION_STATES.RETAINED);
    expect(classifyReviewOutcome({ accuracy: 70, skillId: 'VL002' }).retentionStatus).toBe(RETENTION_STATES.NEEDS_REVIEW);
    expect(classifyReviewOutcome({ accuracy: 30, skillId: 'VL002' }).retentionStatus).toBe(RETENTION_STATES.FORGOTTEN);
  });

  it('advances the schedule to the next interval when retained', () => {
    const completion = applyReviewCompletion({ skillId: 'VL002', accuracy: 95, averageTimeSeconds: 6, lastIntervalDays: 3 });
  ...
  it('re-reviews soon (shortest interval) when not retained', () => {
    const completion = applyReviewCompletion({ skillId: 'VL002', accuracy: 40, lastIntervalDays: 7 });
    expect(completion.set.retentionStatus).toBe(RETENTION_STATES.FORGOTTEN);
    expect(completion.nextIntervalDays).toBe(3);
  });
});

describe('Volume retention — self-validation', () => {
  it('passes the engine self-test', () => {
    const result = validateVolumeRetentionEngine();
