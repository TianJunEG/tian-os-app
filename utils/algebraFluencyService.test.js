...
describe('Algebra fluency — benchmarks + classification', () => {
...
describe('Algebra fluency — drill building', () => {
  it('builds a timed drill for a skill with benchmarks and questions', () => {
    const drill = buildAlgebraFluencyDrill({ skillId: 'AL003', count: 8 });
    expect(drill.domainId).toBe('algebra');
    expect(drill.skillId).toBe('AL003');
    expect(drill.benchmarks.gold).toBeGreaterThan(0);
    expect(drill.questions).toHaveLength(8);
    expect(drill.questions.every((q) => q.skillId === 'AL003')).toBe(true);
    expect(new Set(drill.questions.map((q) => q.questionId)).size).toBe(8);
  });

  it('strips answers from the client copy', () => {
    const drill = buildAlgebraFluencyDrill({ skillId: 'AL006', count: 4 });
  ...
  it('rejects an unknown skill', () => {
    expect(() => buildAlgebraFluencyDrill({ skillId: 'ZZZ' })).toThrow();
  });
});

describe('Algebra fluency — scoring', () => {
  it('awards a high band for fast, fully-correct answers', () => {
    ...
    const scored = scoreAlgebraFluencyDrill({ skillId: 'AL003', questions: drill.questions, responses: fast });
  ...
  it('returns notReady for slow or inaccurate drills', () => {
    const drill = buildAlgebraFluencyDrill({ skillId: 'AL006', count: 8 });
    const slowWrong = drill.questions.map((q, i) => ({
      questionId: q.questionId,
      studentAnswer: i % 2 === 0 ? q.answer.display : 'wrong',
      timeTaken: 60,
    }));
    const scored = scoreAlgebraFluencyDrill({ skillId: 'AL006', questions: drill.questions, responses: slowWrong });
    expect(scored.band).toBe('notReady');
    expect(scored.accuracy).toBeLessThan(80);
  });

  it('ignores responses for unknown question ids', () => {
    const drill = buildAlgebraFluencyDrill({ skillId: 'AL001', count: 3 });
    const scored = scoreAlgebraFluencyDrill({
      skillId: 'AL001',
