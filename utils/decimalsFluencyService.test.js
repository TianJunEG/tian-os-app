...
describe('Decimals fluency — benchmarks + classification', () => {
...
describe('Decimals fluency — drill building', () => {
  it('builds a timed drill for a skill with benchmarks and questions', () => {
    const drill = buildDecimalsFluencyDrill({ skillId: 'D003', count: 8 });
    expect(drill.domainId).toBe('decimals');
    expect(drill.skillId).toBe('D003');
    expect(drill.benchmarks.gold).toBeGreaterThan(0);
    expect(drill.questions).toHaveLength(8);
    expect(drill.questions.every((q) => q.skillId === 'D003')).toBe(true);
    expect(new Set(drill.questions.map((q) => q.questionId)).size).toBe(8);
  });

  it('strips answers from the client copy', () => {
    const drill = buildDecimalsFluencyDrill({ skillId: 'D007', count: 4 });
  ...
  it('rejects an unknown skill', () => {
    expect(() => buildDecimalsFluencyDrill({ skillId: 'ZZZ' })).toThrow();
  });
});

describe('Decimals fluency — scoring', () => {
  it('awards a high band for fast, fully-correct answers', () => {
    ...
    const scored = scoreDecimalsFluencyDrill({ skillId: 'D003', questions: drill.questions, responses: fast });
  ...
  it('returns notReady for slow or inaccurate drills', () => {
    const drill = buildDecimalsFluencyDrill({ skillId: 'D006', count: 8 });
    const slowWrong = drill.questions.map((q, i) => ({
      questionId: q.questionId,
      studentAnswer: i % 2 === 0 ? q.answer.display : 'wrong',
      timeTaken: 60,
    }));
    const scored = scoreDecimalsFluencyDrill({ skillId: 'D006', questions: drill.questions, responses: slowWrong });
    expect(scored.band).toBe('notReady');
    expect(scored.accuracy).toBeLessThan(80);
  });

  it('ignores responses for unknown question ids', () => {
    const drill = buildDecimalsFluencyDrill({ skillId: 'D001', count: 3 });
    const scored = scoreDecimalsFluencyDrill({
      skillId: 'D001',
