...
describe('Percentage fluency — scoring', () => {
  it('awards a high band for fast, fully-correct answers', () => {
    ...
    const scored = scorePercentageFluencyDrill({ skillId: 'P001', questions: drill.questions, responses: fast });
    expect(scored.accuracy).toBe(100);
    expect(scored.domainId).toBe('percentage');
  ...
  it('returns notReady for slow or inaccurate drills', () => {
    const drill = buildPercentageFluencyDrill({ skillId: 'P002', count: 8 });
    const slowWrong = drill.questions.map((q, i) => ({
      questionId: q.questionId,
      studentAnswer: i % 2 === 0 ? q.acceptedAnswers[0] : 'definitely-wrong',
      timeTaken: 90,
    }));
    const scored = scorePercentageFluencyDrill({ skillId: 'P002', questions: drill.questions, responses: slowWrong });
    expect(scored.band).toBe('notReady');
    expect(scored.accuracy).toBeLessThan(80);
  });

  it('ignores responses for unknown question ids', () => {
    const drill = buildPercentageFluencyDrill({ skillId: 'P001', count: 3 });
