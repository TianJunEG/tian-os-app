...
describe('Percentage fluency — benchmarks + classification', () => {
...
describe('Percentage fluency — drill building', () => {
  it('builds a timed drill for a skill with benchmarks and questions', () => {
    const drill = buildPercentageFluencyDrill({ skillId: 'P003', count: 8 });
    expect(drill.domainId).toBe('percentage');
    expect(drill.skillId).toBe('P003');
    expect(drill.benchmarks.gold).toBeGreaterThan(0);
    expect(drill.questions).toHaveLength(8);
    expect(drill.questions.every((q) => q.skillId === 'P003')).toBe(true);
    expect(new Set(drill.questions.map((q) => q.questionId)).size).toBe(8);
  });

  it('strips answers from the client copy', () => {
    const drill = buildPercentageFluencyDrill({ skillId: 'P001', count: 4 });
  ...
  it('rejects an unknown skill', () => {
    expect(() => buildPercentageFluencyDrill({ skillId: 'ZZZ' })).toThrow();
  });
});

describe('Percentage fluency — scoring', () => {
  it('awards a high band for fast, fully-correct answers', () => {
    const drill = buildPercentageFluencyDrill({ skillId: 'P003', count: 8 });
    const fast = drill.questions.map((q) => ({ questionId: q.questionId, studentAnswer: q.answer.display ?? q.answer, timeTaken: 3 }));
    const scored = scorePercentageFluencyDrill({ skillId: 'P003', questions: drill.questions, responses: fast });
  ...
  it('returns notReady for slow or inaccurate drills', () => {
    const drill = buildPercentageFluencyDrill({ skillId: 'P001', count: 8 });
    const slowWrong = drill.questions.map((q, i) => ({
      questionId: q.questionId,
      studentAnswer: i % 2 === 0 ? (q.answer.display ?? q.answer) : 'wrong',
      timeTaken: 60,
    }));
    const scored = scorePercentageFluencyDrill({ skillId: 'P001', questions: drill.questions, responses: slowWrong });
    expect(scored.band).toBe('notReady');
    expect(scored.accuracy).toBeLessThan(80);
  });

  it('ignores responses for unknown question ids', () => {
    const drill = buildPercentageFluencyDrill({ skillId: 'P002', count: 3 });
    const scored = scorePercentageFluencyDrill({
      skillId: 'P002',
