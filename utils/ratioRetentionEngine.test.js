...
import {
  ...
  validateRatioRetentionEngine,
} from '../shared/mathpath/ratio/ratioRetentionEngine.js';

const FLUENT_AT = '2026-01-01T00:00:00.000Z';

describe('Ratio retention — scheduling', () => {
  it('reads review intervals from the skill graph (ratio ships [3,7,30,90])', () => {
    expect(reviewIntervalsForSkill('R003')).toEqual([3, 7, 30, 90]);
  });

  it('builds dated reviews from the fluent-at anchor using the skill intervals', () => {
    const schedule = buildSpacedReviewSchedule({ skillId: 'R003', fluentAt: FLUENT_AT });
...
describe('Ratio retention — mastery gate', () => {
  ...
  it('blocks scheduling for non-fluent skills, allows it for fluent ones', () => {
    const blocked = buildRetentionScheduleFromFluency({ skillId: 'R003', fluencyLevel: 'bronze', fluentAt: FLUENT_AT });
    expect(blocked.shouldSchedule).toBe(false);
    expect(blocked.reviews).toHaveLength(0);

    const allowed = buildRetentionScheduleFromFluency({ skillId: 'R003', fluencyLevel: 'gold', fluentAt: FLUENT_AT });
...
describe('Ratio retention — due detection', () => {
  it('finds the earliest passed-but-uncompleted review', () => {
    const due = findNextDueReview({ skillId: 'R003', fluentAt: FLUENT_AT, completedIntervalDays: [], asOf: '2026-01-05T00:00:00.000Z' });
    expect(due.intervalDays).toBe(3);
  });

  it('skips reviews already completed', () => {
    const due = findNextDueReview({ skillId: 'R003', fluentAt: FLUENT_AT, completedIntervalDays: [3], asOf: '2026-01-09T00:00:00.000Z' });
    expect(due.intervalDays).toBe(7);
  });

  it('returns null before any review is due', () => {
    const due = findNextDueReview({ skillId: 'R003', fluentAt: FLUENT_AT, completedIntervalDays: [], asOf: '2026-01-02T00:00:00.000Z' });
    expect(due).toBeNull();
  });

  it('marks a fluent skill as review_due once the first interval has passed', () => {
    const status = evaluateRetentionStatus({
      state: { skillId: 'R003', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
  ...
  it('detectForgetting surfaces only skills whose retention is at risk', () => {
    const alerts = detectForgetting({
      states: [
        { skillId: 'R003', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
        { skillId: 'R004', fluentAt: '2026-01-07T00:00:00.000Z', fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
      ],
      asOf: '2026-01-08T00:00:00.000Z',
    });
...
describe('Ratio retention — summary shape', () => {
  it('buckets skill-states into upcoming / overdue / history', () => {
    const summary = summariseRetention({
      states: [
        // upcoming: fluent but not yet due
        { skillId: 'R001', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled', nextReviewDate: '2026-02-01T00:00:00.000Z' },
        // overdue: due date passed
        { skillId: 'R003', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'fluent', retentionStatus: 'reviewScheduled' },
        // history: retained
        { skillId: 'R005', fluentAt: FLUENT_AT, fluencyLevel: 'gold', status: 'retained', retentionStatus: 'retained' },
      ],
      asOf: '2026-01-15T00:00:00.000Z',
    });
    expect(summary.domainId).toBe('ratio');
    expect(summary.upcomingReviews.map((r) => r.skillId)).toContain('R001');
    expect(summary.overdueReviews.map((r) => r.skillId)).toContain('R003');
    expect(summary.retentionHistory.map((r) => r.skillId)).toContain('R005');
  });
...
describe('Ratio retention — review generation', () => {
  it('produces a same-concept review using different question families', () => {
    const review = generateRetentionReview({ skillId: 'R003', previousQuestionFamilyIds: ['QF_R003_001'] });
    expect(review.skillId).toBe('R003');
    expect(review.sameConcept).toBe(true);
    expect(review.questionFamilyIds).not.toContain('QF_R003_001');
    expect(review.questionFamilyIds.length).toBeGreaterThan(0);
  });
});

describe('Ratio retention — outcome classification + completion', () => {
  it('classifies retained / needs-review / forgotten by accuracy + speed', () => {
    expect(classifyReviewOutcome({ accuracy: 95, averageTimeSeconds: 6, skillId: 'R003' }).retentionStatus).toBe(RETENTION_STATES.RETAINED);
    expect(classifyReviewOutcome({ accuracy: 70, skillId: 'R003' }).retentionStatus).toBe(RETENTION_STATES.NEEDS_REVIEW);
    expect(classifyReviewOutcome({ accuracy: 30, skillId: 'R003' }).retentionStatus).toBe(RETENTION_STATES.FORGOTTEN);
  });

  it('advances the schedule to the next interval when retained', () => {
    const completion = applyReviewCompletion({ skillId: 'R003', accuracy: 95, averageTimeSeconds: 6, lastIntervalDays: 3 });
  ...
  it('re-reviews soon (shortest interval) when not retained', () => {
    const completion = applyReviewCompletion({ skillId: 'R003', accuracy: 40, lastIntervalDays: 7 });
    expect(completion.set.retentionStatus).toBe(RETENTION_STATES.FORGOTTEN);
    expect(completion.nextIntervalDays).toBe(3);
  });
});

describe('Ratio retention — self-validation', () => {
  it('passes the engine self-test', () => {
    const result = validateRatioRetentionEngine();
